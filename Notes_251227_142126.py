"""
Школьная столовая
Полная система управления питанием с регистрацией и входом для всех ролей
"""

from flask import Flask, jsonify, request, session, redirect, g, render_template_string
import sqlite3
import uuid
import os
import json
from datetime import date, datetime, timedelta
from werkzeug.security import generate_password_hash, check_password_hash
import calendar
import re

# ==================== НАСТРОЙКИ ====================

app = Flask(__name__)
app.secret_key = 'школа-столовая-2024-секрет-ключ'
app.config['DATABASE'] = 'школа_столовая_новая.db'

# ==================== БАЗА ДАННЫХ ====================

def get_db():
    """Подключение к БД"""
    if 'db' not in g:
        g.db = sqlite3.connect(app.config['DATABASE'])
        g.db.row_factory = sqlite3.Row
    return g.db

def init_db():
    """Инициализация БД с правильной структурой"""
    with app.app_context():
        db = get_db()
        
        print("🔄 Создаем базу данных...")
        
        # Удаляем старые таблицы если есть
        db.execute('DROP TABLE IF EXISTS subscriptions')
        db.execute('DROP TABLE IF EXISTS issued_meals')
        db.execute('DROP TABLE IF EXISTS reviews')
        db.execute('DROP TABLE IF EXISTS orders')
        db.execute('DROP TABLE IF EXISTS menu')
        db.execute('DROP TABLE IF EXISTS users')
        
        # Создаем таблицу пользователей с номером телефона
        db.execute('''
            CREATE TABLE users (
                id TEXT PRIMARY KEY,
                email TEXT UNIQUE NOT NULL,
                password TEXT NOT NULL,
                first_name TEXT NOT NULL,
                last_name TEXT NOT NULL,
                phone TEXT,
                class_name TEXT,
                role TEXT NOT NULL DEFAULT 'student',
                position TEXT,
                balance REAL DEFAULT 1000,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
        ''')
        
        # Создаем таблицу меню
        db.execute('''
            CREATE TABLE menu (
                id TEXT PRIMARY KEY,
                day TEXT NOT NULL,
                name TEXT NOT NULL,
                description TEXT,
                price REAL NOT NULL,
                meal_type TEXT NOT NULL,
                available BOOLEAN DEFAULT 1,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
        ''')
        
        # Создаем таблицу заказов
        db.execute('''
            CREATE TABLE orders (
                id TEXT PRIMARY KEY,
                user_id TEXT NOT NULL,
                menu_id TEXT NOT NULL,
                status TEXT DEFAULT 'ожидает оплаты',
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY (user_id) REFERENCES users(id),
                FOREIGN KEY (menu_id) REFERENCES menu(id)
            )
        ''')
        
        # Создаем таблицу отзывов
        db.execute('''
            CREATE TABLE reviews (
                id TEXT PRIMARY KEY,
                user_id TEXT NOT NULL,
                menu_id TEXT NOT NULL,
                rating INTEGER NOT NULL CHECK (rating >= 1 AND rating <= 5),
                comment TEXT,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY (user_id) REFERENCES users(id),
                FOREIGN KEY (menu_id) REFERENCES menu(id)
            )
        ''')
        
        # Создаем таблицу абонементов
        db.execute('''
            CREATE TABLE subscriptions (
                id TEXT PRIMARY KEY,
                user_id TEXT NOT NULL,
                menu_id TEXT NOT NULL,
                selected_dates TEXT NOT NULL, -- JSON массив выбранных дат
                start_date TEXT NOT NULL,
                end_date TEXT NOT NULL,
                total_price REAL NOT NULL,
                status TEXT DEFAULT 'активен',
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY (user_id) REFERENCES users(id),
                FOREIGN KEY (menu_id) REFERENCES menu(id)
            )
        ''')
        
        # Создаем таблицу выданных блюд
        db.execute('''
            CREATE TABLE issued_meals (
                id TEXT PRIMARY KEY,
                user_id TEXT NOT NULL,
                menu_id TEXT NOT NULL,
                subscription_id TEXT,
                issue_date TEXT NOT NULL,
                meal_type TEXT NOT NULL,
                issued_by TEXT, -- ID повара, который выдал
                status TEXT DEFAULT 'ожидает выдачи',
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY (user_id) REFERENCES users(id),
                FOREIGN KEY (menu_id) REFERENCES menu(id),
                FOREIGN KEY (subscription_id) REFERENCES subscriptions(id)
            )
        ''')
        
        # Добавляем тестовое меню на месяц вперед
        today = date.today()
        menu_items = []
        
        # Добавляем меню на 30 дней вперед
        for i in range(30):
            current_date = today + timedelta(days=i)
            date_str = current_date.strftime('%Y-%m-%d')
            
            # Завтраки
            menu_items.append((date_str, 'Каша овсяная с ягодами', 'Полезный завтрак', 120, 'завтрак'))
            menu_items.append((date_str, 'Сырники со сметаной', 'Домашние сырники', 140, 'завтрак'))
            menu_items.append((date_str, 'Омлет с ветчиной', 'Сытный завтрак', 150, 'завтрак'))
            
            # Обеды
            menu_items.append((date_str, 'Суп куриный с лапшой', 'Ароматный суп', 150, 'обед'))
            menu_items.append((date_str, 'Гречка с котлетой', 'Сытный обед', 180, 'обед'))
            menu_items.append((date_str, 'Плов с говядиной', 'Традиционный плов', 200, 'обед'))
            menu_items.append((date_str, 'Салат овощной', 'Свежие овощи', 90, 'обед'))
            
            # Полдники
            menu_items.append((date_str, 'Творожная запеканка', 'Нежный десерт', 130, 'полдник'))
            menu_items.append((date_str, 'Йогурт с фруктами', 'Легкий полдник', 100, 'полдник'))
        
        for item in menu_items:
            db.execute('''
                INSERT INTO menu (id, day, name, description, price, meal_type)
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (str(uuid.uuid4()), *item))
        
        db.commit()
        print("✅ База данных создана успешно!")

# ==================== ВАЛИДАЦИЯ ТЕЛЕФОНА ====================

def validate_phone(phone):
    """Валидация номера телефона"""
    if not phone:
        return False
    
    # Убираем все нецифровые символы кроме плюса
    cleaned = re.sub(r'[^\d+]', '', phone)
    
    # Проверяем разные форматы телефонов
    # Россия: +7XXXXXXXXXX, 8XXXXXXXXXX
    if cleaned.startswith('+7') and len(cleaned) == 12:
        return True
    elif cleaned.startswith('8') and len(cleaned) == 11:
        return True
    elif len(cleaned) == 10 and cleaned.startswith('9'):  # Без кода страны
        return True
    elif len(cleaned) >= 10 and len(cleaned) <= 15:  # Международный формат
        return True
    
    return False

def format_phone(phone):
    """Форматирование телефона в единый формат"""
    if not phone:
        return phone
    
    # Убираем все нецифровые символы
    cleaned = re.sub(r'[^\d]', '', phone)
    
    if len(cleaned) == 11 and cleaned.startswith('8'):
        # Преобразуем 8 в +7
        return '+7' + cleaned[1:]
    elif len(cleaned) == 10:
        # Добавляем +7 для 10-значных номеров
        return '+7' + cleaned
    else:
        # Возвращаем как есть с плюсом
        if not cleaned.startswith('+'):
            return '+' + cleaned
        return cleaned

# ==================== ГЛАВНАЯ СТРАНИЦА ====================

@app.route('/')
def home():
    """Главная страница"""
    return '''
    <!DOCTYPE html>
    <html>
    <head>
        <title>Школьная столовая</title>
        <meta charset="utf-8">
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            body { 
                font-family: 'Arial', sans-serif; 
                background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
                min-height: 100vh;
                padding: 20px;
                color: white;
            }
            .container {
                max-width: 900px;
                margin: 0 auto;
                background: rgba(255, 255, 255, 0.95);
                padding: 40px;
                border-radius: 20px;
                box-shadow: 0 20px 60px rgba(0,0,0,0.3);
                color: #333;
            }
            h1 {
                color: #2c3e50;
                text-align: center;
                margin-bottom: 10px;
                font-size: 2.5em;
            }
            .subtitle {
                text-align: center;
                color: #7f8c8d;
                margin-bottom: 40px;
                font-size: 1.2em;
            }
            .role-grid {
                display: grid;
                grid-template-columns: repeat(auto-fit, minmax(250px, 1fr));
                gap: 20px;
                margin: 40px 0;
            }
            .role-card {
                background: white;
                border-radius: 15px;
                padding: 30px;
                text-align: center;
                text-decoration: none;
                color: #333;
                transition: all 0.3s;
                border: 2px solid transparent;
                box-shadow: 0 5px 15px rgba(0,0,0,0.1);
            }
            .role-card:hover {
                transform: translateY(-10px);
                box-shadow: 0 15px 30px rgba(0,0,0,0.2);
                border-color: #3498db;
            }
            .role-icon {
                font-size: 3em;
                margin-bottom: 15px;
            }
            .role-name {
                font-size: 1.5em;
                font-weight: bold;
                margin-bottom: 10px;
                color: #2c3e50;
            }
            .role-desc {
                color: #7f8c8d;
                font-size: 0.9em;
            }
            .info-box {
                background: #e8f4fc;
                padding: 20px;
                border-radius: 10px;
                margin-top: 30px;
                text-align: center;
            }
        </style>
    </head>
    <body>
        <div class="container">
            <h1>🏫 Школьная столовая</h1>
            <p class="subtitle">Система управления питанием в учебном заведении</p>
            
            <div class="role-grid">
                <a href="/student" class="role-card">
                    <div class="role-icon">👨‍🎓</div>
                    <div class="role-name">Ученик</div>
                    <div class="role-desc">Регистрация, вход, заказ питания, абонементы, отзывы</div>
                </a>
                
                <a href="/chef" class="role-card">
                    <div class="role-icon">👨‍🍳</div>
                    <div class="role-name">Повар</div>
                    <div class="role-desc">Регистрация и вход для сотрудников, учет выдачи</div>
                </a>
                
                <a href="/admin" class="role-card">
                    <div class="role-icon">👨‍💼</div>
                    <div class="role-name">Администратор</div>
                    <div class="role-desc">Регистрация и вход для администрации, статистика</div>
                </a>
            </div>
            
            <div class="info-box">
                <h3>📱 Новая функция!</h3>
                <p><strong>Добавлен номер телефона</strong> при регистрации для лучшей связи.</p>
                <p>Все пользователи должны зарегистрироваться самостоятельно.</p>
            </div>
        </div>
    </body>
    </html>
    '''

# ==================== СТРАНИЦА УЧЕНИКА ====================

@app.route('/student')
def student_page():
    """Страница выбора: вход или регистрация для ученика"""
    return '''
    <!DOCTYPE html>
    <html>
    <head>
        <title>Вход для ученика</title>
        <meta charset="utf-8">
        <style>
            body { 
                font-family: 'Arial', sans-serif; 
                background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
                min-height: 100vh;
                display: flex;
                justify-content: center;
                align-items: center;
                padding: 20px;
            }
            .container {
                background: white;
                padding: 40px;
                border-radius: 20px;
                box-shadow: 0 20px 60px rgba(0,0,0,0.3);
                max-width: 500px;
                width: 100%;
            }
            h1 {
                color: #2c3e50;
                text-align: center;
                margin-bottom: 30px;
            }
            .btn-group {
                display: flex;
                flex-direction: column;
                gap: 15px;
            }
            .btn {
                padding: 15px;
                border: none;
                border-radius: 10px;
                font-size: 1.1em;
                cursor: pointer;
                transition: all 0.3s;
                text-align: center;
                text-decoration: none;
                display: block;
            }
            .btn-login {
                background: #3498db;
                color: white;
            }
            .btn-login:hover {
                background: #2980b9;
            }
            .btn-register {
                background: #2ecc71;
                color: white;
            }
            .btn-register:hover {
                background: #27ae60;
            }
            .btn-back {
                background: #95a5a6;
                color: white;
                margin-top: 20px;
            }
            .btn-back:hover {
                background: #7f8c8d;
            }
        </style>
    </head>
    <body>
        <div class="container">
            <h1>👨‍🎓 Вход для ученика</h1>
            <p style="text-align: center; margin-bottom: 30px; color: #7f8c8d;">
                Выберите действие:
            </p>
            
            <div class="btn-group">
                <a href="/student/login" class="btn btn-login">Войти в аккаунт</a>
                <a href="/student/register" class="btn btn-register">Зарегистрироваться</a>
                <a href="/" class="btn btn-back">← На главную</a>
            </div>
        </div>
    </body>
    </html>
    '''

# ==================== РЕГИСТРАЦИЯ УЧЕНИКА ====================

@app.route('/student/register', methods=['GET', 'POST'])
def student_register():
    """Регистрация нового ученика с номером телефона"""
    if request.method == 'POST':
        # Получаем данные из формы
        email = request.form.get('email', '').strip()
        password = request.form.get('password', '').strip()
        confirm_password = request.form.get('confirm_password', '').strip()
        first_name = request.form.get('first_name', '').strip()
        last_name = request.form.get('last_name', '').strip()
        phone = request.form.get('phone', '').strip()
        class_name = request.form.get('class_name', '').strip()
        
        # Проверяем обязательные поля
        if not all([email, password, confirm_password, first_name, last_name, phone, class_name]):
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Все поля обязательны для заполнения!</p>
                <a href="/student/register" style="color: #3498db;">← Вернуться к регистрации</a>
            </body>
            </html>
            '''
        
        # Проверяем совпадение паролей
        if password != confirm_password:
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Пароли не совпадают!</p>
                <a href="/student/register" style="color: #3498db;">← Вернуться к регистрации</a>
            </body>
            </html>
            '''
        
        # Проверяем email
        if '@' not in email or '.' not in email:
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Введите корректный email адрес!</p>
                <a href="/student/register" style="color: #3498db;">← Вернуться к регистрации</a>
            </body>
            </html>
            '''
        
        # Проверяем номер телефона
        if not validate_phone(phone):
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Введите корректный номер телефона!</p>
                <p>Формат: +7XXXXXXXXXX или 8XXXXXXXXXX</p>
                <a href="/student/register" style="color: #3498db;">← Вернуться к регистрации</a>
            </body>
            </html>
            '''
        
        # Форматируем телефон
        formatted_phone = format_phone(phone)
        
        db = get_db()
        
        # Проверяем, нет ли уже такого email
        existing_user = db.execute('SELECT id FROM users WHERE email = ?', (email,)).fetchone()
        if existing_user:
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Пользователь с таким email уже зарегистрирован!</p>
                <a href="/student/register" style="color: #3498db;">← Вернуться к регистрации</a>
            </body>
            </html>
            '''
        
        # Проверяем, нет ли уже такого телефона
        existing_phone = db.execute('SELECT id FROM users WHERE phone = ?', (formatted_phone,)).fetchone()
        if existing_phone:
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Пользователь с таким номером телефона уже зарегистрирован!</p>
                <a href="/student/register" style="color: #3498db;">← Вернуться к регистрации</a>
            </body>
            </html>
            '''
        
        # Создаем нового пользователя
        user_id = str(uuid.uuid4())
        hashed_password = generate_password_hash(password)
        
        db.execute('''
            INSERT INTO users (id, email, password, first_name, last_name, phone, class_name, role, balance)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (user_id, email, hashed_password, first_name, last_name, formatted_phone, class_name, 'student', 1000))
        
        db.commit()
        
        # Автоматически входим после регистрации
        session['user_id'] = user_id
        session['user_email'] = email
        session['user_name'] = f"{first_name} {last_name}"
        session['user_role'] = 'student'
        session['user_class'] = class_name
        
        return redirect('/dashboard')
    
    # GET запрос - показываем форму регистрации
    return '''
    <!DOCTYPE html>
    <html>
    <head>
        <title>Регистрация ученика</title>
        <meta charset="utf-8">
        <style>
            body { 
                font-family: 'Arial', sans-serif; 
                background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
                min-height: 100vh;
                display: flex;
                justify-content: center;
                align-items: center;
                padding: 20px;
            }
            .container {
                background: white;
                padding: 40px;
                border-radius: 20px;
                box-shadow: 0 20px 60px rgba(0,0,0,0.3);
                max-width: 500px;
                width: 100%;
            }
            h1 {
                color: #2c3e50;
                text-align: center;
                margin-bottom: 30px;
            }
            .form-group {
                margin-bottom: 20px;
            }
            label {
                display: block;
                margin-bottom: 8px;
                color: #2c3e50;
                font-weight: bold;
            }
            input {
                width: 100%;
                padding: 12px;
                border: 2px solid #ddd;
                border-radius: 8px;
                font-size: 1em;
                transition: border-color 0.3s;
            }
            input:focus {
                border-color: #3498db;
                outline: none;
            }
            .help-text {
                font-size: 0.85em;
                color: #7f8c8d;
                margin-top: 5px;
            }
            .btn {
                width: 100%;
                padding: 15px;
                border: none;
                border-radius: 10px;
                font-size: 1.1em;
                cursor: pointer;
                transition: all 0.3s;
                margin-top: 10px;
            }
            .btn-register {
                background: #2ecc71;
                color: white;
            }
            .btn-register:hover {
                background: #27ae60;
            }
            .btn-back {
                background: #95a5a6;
                color: white;
            }
            .btn-back:hover {
                background: #7f8c8d;
            }
        </style>
    </head>
    <body>
        <div class="container">
            <h1>📝 Регистрация ученика</h1>
            
            <form method="POST" action="/student/register">
                <div class="form-group">
                    <label for="email">Email:</label>
                    <input type="email" id="email" name="email" required placeholder="example@school.ru">
                </div>
                
                <div class="form-group">
                    <label for="password">Пароль:</label>
                    <input type="password" id="password" name="password" required minlength="6" placeholder="Не менее 6 символов">
                </div>
                
                <div class="form-group">
                    <label for="confirm_password">Повторите пароль:</label>
                    <input type="password" id="confirm_password" name="confirm_password" required placeholder="Повторите пароль">
                </div>
                
                <div class="form-group">
                    <label for="first_name">Имя:</label>
                    <input type="text" id="first_name" name="first_name" required placeholder="Иван">
                </div>
                
                <div class="form-group">
                    <label for="last_name">Фамилия:</label>
                    <input type="text" id="last_name" name="last_name" required placeholder="Иванов">
                </div>
                
                <div class="form-group">
                    <label for="phone">Номер телефона:</label>
                    <input type="tel" id="phone" name="phone" required placeholder="+7XXXXXXXXXX или 8XXXXXXXXXX">
                    <div class="help-text">Формат: +7XXX XXX XX XX или 8XXX XXX XX XX</div>
                </div>
                
                <div class="form-group">
                    <label for="class_name">Класс:</label>
                    <input type="text" id="class_name" name="class_name" required placeholder="10А">
                </div>
                
                <button type="submit" class="btn btn-register">Зарегистрироваться</button>
                <a href="/student" class="btn btn-back" style="text-decoration: none; display: block; text-align: center;">← Назад</a>
            </form>
        </div>
    </body>
    </html>
    '''

# ==================== ВХОД УЧЕНИКА ====================

@app.route('/student/login', methods=['GET', 'POST'])
def student_login():
    """Вход для ученика"""
    if request.method == 'POST':
        email = request.form.get('email', '').strip()
        password = request.form.get('password', '').strip()
        
        if not email or not password:
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Заполните все поля!</p>
                <a href="/student/login" style="color: #3498db;">← Вернуться ко входу</a>
            </body>
            </html>
            '''
        
        db = get_db()
        user = db.execute('SELECT * FROM users WHERE email = ? AND role = ?', (email, 'student')).fetchone()
        
        if user and check_password_hash(user['password'], password):
            session['user_id'] = user['id']
            session['user_email'] = email
            session['user_name'] = f"{user['first_name']} {user['last_name']}"
            session['user_role'] = user['role']
            session['user_class'] = user['class_name']
            return redirect('/dashboard')
        else:
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Неверный email или пароль!</p>
                <a href="/student/login" style="color: #3498db;">← Попробовать снова</a>
            </body>
            </html>
            '''
    
    # GET запрос - показываем форму входа
    return '''
    <!DOCTYPE html>
    <html>
    <head>
        <title>Вход для ученика</title>
        <meta charset="utf-8">
        <style>
            body { 
                font-family: 'Arial', sans-serif; 
                background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
                min-height: 100vh;
                display: flex;
                justify-content: center;
                align-items: center;
                padding: 20px;
            }
            .container {
                background: white;
                padding: 40px;
                border-radius: 20px;
                box-shadow: 0 20px 60px rgba(0,0,0,0.3);
                max-width: 500px;
                width: 100%;
            }
            h1 {
                color: #2c3e50;
                text-align: center;
                margin-bottom: 30px;
            }
            .form-group {
                margin-bottom: 20px;
            }
            label {
                display: block;
                margin-bottom: 8px;
                color: #2c3e50;
                font-weight: bold;
            }
            input {
                width: 100%;
                padding: 12px;
                border: 2px solid #ddd;
                border-radius: 8px;
                font-size: 1em;
                transition: border-color 0.3s;
            }
            input:focus {
                border-color: #3498db;
                outline: none;
            }
            .btn {
                width: 100%;
                padding: 15px;
                border: none;
                border-radius: 10px;
                font-size: 1.1em;
                cursor: pointer;
                transition: all 0.3s;
                margin-top: 10px;
            }
            .btn-login {
                background: #3498db;
                color: white;
            }
            .btn-login:hover {
                background: #2980b9;
            }
            .btn-back {
                background: #95a5a6;
                color: white;
            }
            .btn-back:hover {
                background: #7f8c8d;
            }
            .register-link {
                text-align: center;
                margin-top: 20px;
            }
        </style>
    </head>
    <body>
        <div class="container">
            <h1>🔐 Вход для ученика</h1>
            
            <form method="POST" action="/student/login">
                <div class="form-group">
                    <label for="email">Email:</label>
                    <input type="email" id="email" name="email" required placeholder="Введите ваш email">
                </div>
                
                <div class="form-group">
                    <label for="password">Пароль:</label>
                    <input type="password" id="password" name="password" required placeholder="Введите пароль">
                </div>
                
                <button type="submit" class="btn btn-login">Войти</button>
                <a href="/student" class="btn btn-back" style="text-decoration: none; display: block; text-align: center;">← Назад</a>
            </form>
            
            <div class="register-link">
                <p>Нет аккаунта? <a href="/student/register">Зарегистрируйтесь</a></p>
            </div>
        </div>
    </body>
    </html>
    '''

# ==================== СТРАНИЦА ПОВАРА ====================

@app.route('/chef')
def chef_page():
    """Страница выбора: вход или регистрация для повара"""
    return '''
    <!DOCTYPE html>
    <html>
    <head>
        <title>Вход для повара</title>
        <meta charset="utf-8">
        <style>
            body { 
                font-family: 'Arial', sans-serif; 
                background: linear-gradient(135deg, #f39c12 0%, #e67e22 100%);
                min-height: 100vh;
                display: flex;
                justify-content: center;
                align-items: center;
                padding: 20px;
            }
            .container {
                background: white;
                padding: 40px;
                border-radius: 20px;
                box-shadow: 0 20px 60px rgba(0,0,0,0.3);
                max-width: 500px;
                width: 100%;
            }
            h1 {
                color: #2c3e50;
                text-align: center;
                margin-bottom: 30px;
            }
            .btn-group {
                display: flex;
                flex-direction: column;
                gap: 15px;
            }
            .btn {
                padding: 15px;
                border: none;
                border-radius: 10px;
                font-size: 1.1em;
                cursor: pointer;
                transition: all 0.3s;
                text-align: center;
                text-decoration: none;
                display: block;
            }
            .btn-login {
                background: #e67e22;
                color: white;
            }
            .btn-login:hover {
                background: #d35400;
            }
            .btn-register {
                background: #f39c12;
                color: white;
            }
            .btn-register:hover {
                background: #e67e22;
            }
            .btn-back {
                background: #95a5a6;
                color: white;
                margin-top: 20px;
            }
            .btn-back:hover {
                background: #7f8c8d;
            }
        </style>
    </head>
    <body>
        <div class="container">
            <h1>👨‍🍳 Вход для повара</h1>
            <p style="text-align: center; margin-bottom: 30px; color: #7f8c8d;">
                Выберите действие:
            </p>
            
            <div class="btn-group">
                <a href="/chef/login" class="btn btn-login">Войти в аккаунт</a>
                <a href="/chef/register" class="btn btn-register">Зарегистрироваться</a>
                <a href="/" class="btn btn-back">← На главную</a>
            </div>
        </div>
    </body>
    </html>
    '''

# ==================== РЕГИСТРАЦИЯ ПОВАРА ====================

@app.route('/chef/register', methods=['GET', 'POST'])
def chef_register():
    """Регистрация нового повара с номером телефона"""
    if request.method == 'POST':
        # Получаем данные из формы
        email = request.form.get('email', '').strip()
        password = request.form.get('password', '').strip()
        confirm_password = request.form.get('confirm_password', '').strip()
        first_name = request.form.get('first_name', '').strip()
        last_name = request.form.get('last_name', '').strip()
        phone = request.form.get('phone', '').strip()
        position = request.form.get('position', '').strip()
        
        # Проверяем обязательные поля
        if not all([email, password, confirm_password, first_name, last_name, phone, position]):
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Все поля обязательны для заполнения!</p>
                <a href="/chef/register" style="color: #3498db;">← Вернуться к регистрации</a>
            </body>
            </html>
            '''
        
        # Проверяем совпадение паролей
        if password != confirm_password:
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Пароли не совпадают!</p>
                <a href="/chef/register" style="color: #3498db;">← Вернуться к регистрации</a>
            </body>
            </html>
            '''
        
        # Проверяем email
        if '@' not in email or '.' not in email:
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Введите корректный email адрес!</p>
                <a href="/chef/register" style="color: #3498db;">← Вернуться к регистрации</a>
            </body>
            </html>
            '''
        
        # Проверяем номер телефона
        if not validate_phone(phone):
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Введите корректный номер телефона!</p>
                <p>Формат: +7XXXXXXXXXX или 8XXXXXXXXXX</p>
                <a href="/chef/register" style="color: #3498db;">← Вернуться к регистрации</a>
            </body>
            </html>
            '''
        
        # Форматируем телефон
        formatted_phone = format_phone(phone)
        
        db = get_db()
        
        # Проверяем, нет ли уже такого email
        existing_user = db.execute('SELECT id FROM users WHERE email = ?', (email,)).fetchone()
        if existing_user:
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Пользователь с таким email уже зарегистрирован!</p>
                <a href="/chef/register" style="color: #3498db;">← Вернуться к регистрации</a>
            </body>
            </html>
            '''
        
        # Проверяем, нет ли уже такого телефона
        existing_phone = db.execute('SELECT id FROM users WHERE phone = ?', (formatted_phone,)).fetchone()
        if existing_phone:
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Пользователь с таким номером телефона уже зарегистрирован!</p>
                <a href="/chef/register" style="color: #3498db;">← Вернуться к регистрации</a>
            </body>
            </html>
            '''
        
        # Создаем нового повара
        user_id = str(uuid.uuid4())
        hashed_password = generate_password_hash(password)
        
        db.execute('''
            INSERT INTO users (id, email, password, first_name, last_name, phone, role, position)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (user_id, email, hashed_password, first_name, last_name, formatted_phone, 'chef', position))
        
        db.commit()
        
        # Автоматически входим после регистрации
        session['user_id'] = user_id
        session['user_email'] = email
        session['user_name'] = f"{first_name} {last_name}"
        session['user_role'] = 'chef'
        session['user_position'] = position
        
        return redirect('/dashboard')
    
    # GET запрос - показываем форму регистрации
    return '''
    <!DOCTYPE html>
    <html>
    <head>
        <title>Регистрация повара</title>
        <meta charset="utf-8">
        <style>
            body { 
                font-family: 'Arial', sans-serif; 
                background: linear-gradient(135deg, #f39c12 0%, #e67e22 100%);
                min-height: 100vh;
                display: flex;
                justify-content: center;
                align-items: center;
                padding: 20px;
            }
            .container {
                background: white;
                padding: 40px;
                border-radius: 20px;
                box-shadow: 0 20px 60px rgba(0,0,0,0.3);
                max-width: 500px;
                width: 100%;
            }
            h1 {
                color: #2c3e50;
                text-align: center;
                margin-bottom: 30px;
            }
            .form-group {
                margin-bottom: 20px;
            }
            label {
                display: block;
                margin-bottom: 8px;
                color: #2c3e50;
                font-weight: bold;
            }
            input, select {
                width: 100%;
                padding: 12px;
                border: 2px solid #ddd;
                border-radius: 8px;
                font-size: 1em;
                transition: border-color 0.3s;
            }
            input:focus, select:focus {
                border-color: #e67e22;
                outline: none;
            }
            .help-text {
                font-size: 0.85em;
                color: #7f8c8d;
                margin-top: 5px;
            }
            .btn {
                width: 100%;
                padding: 15px;
                border: none;
                border-radius: 10px;
                font-size: 1.1em;
                cursor: pointer;
                transition: all 0.3s;
                margin-top: 10px;
            }
            .btn-register {
                background: #f39c12;
                color: white;
            }
            .btn-register:hover {
                background: #e67e22;
            }
            .btn-back {
                background: #95a5a6;
                color: white;
            }
            .btn-back:hover {
                background: #7f8c8d;
            }
        </style>
    </head>
    <body>
        <div class="container">
            <h1>📝 Регистрация повара</h1>
            
            <form method="POST" action="/chef/register">
                <div class="form-group">
                    <label for="email">Email:</label>
                    <input type="email" id="email" name="email" required placeholder="chef@school.ru">
                </div>
                
                <div class="form-group">
                    <label for="password">Пароль:</label>
                    <input type="password" id="password" name="password" required minlength="6" placeholder="Не менее 6 символов">
                </div>
                
                <div class="form-group">
                    <label for="confirm_password">Повторите пароль:</label>
                    <input type="password" id="confirm_password" name="confirm_password" required placeholder="Повторите пароль">
                </div>
                
                <div class="form-group">
                    <label for="first_name">Имя:</label>
                    <input type="text" id="first_name" name="first_name" required placeholder="Петр">
                </div>
                
                <div class="form-group">
                    <label for="last_name">Фамилия:</label>
                    <input type="text" id="last_name" name="last_name" required placeholder="Поваров">
                </div>
                
                <div class="form-group">
                    <label for="phone">Номер телефона:</label>
                    <input type="tel" id="phone" name="phone" required placeholder="+7XXXXXXXXXX или 8XXXXXXXXXX">
                    <div class="help-text">Формат: +7XXX XXX XX XX или 8XXX XXX XX XX</div>
                </div>
                
                <div class="form-group">
                    <label for="position">Должность:</label>
                    <select id="position" name="position" required>
                        <option value="">Выберите должность</option>
                        <option value="Шеф-повар">Шеф-повар</option>
                        <option value="Повар">Повар</option>
                        <option value="Помощник повара">Помощник повара</option>
                        <option value="Бармен">Бармен</option>
                    </select>
                </div>
                
                <button type="submit" class="btn btn-register">Зарегистрироваться</button>
                <a href="/chef" class="btn btn-back" style="text-decoration: none; display: block; text-align: center;">← Назад</a>
            </form>
        </div>
    </body>
    </html>
    '''

# ==================== ВХОД ПОВАРА ====================

@app.route('/chef/login', methods=['GET', 'POST'])
def chef_login():
    """Вход для повара"""
    if request.method == 'POST':
        email = request.form.get('email', '').strip()
        password = request.form.get('password', '').strip()
        
        if not email or not password:
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Заполните все поля!</p>
                <a href="/chef/login" style="color: #3498db;">← Вернуться ко входу</a>
            </body>
            </html>
            '''
        
        db = get_db()
        user = db.execute('SELECT * FROM users WHERE email = ? AND role = ?', (email, 'chef')).fetchone()
        
        if user and check_password_hash(user['password'], password):
            session['user_id'] = user['id']
            session['user_email'] = email
            session['user_name'] = f"{user['first_name']} {user['last_name']}"
            session['user_role'] = user['role']
            session['user_position'] = user['position']
            return redirect('/dashboard')
        else:
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Неверный email или пароль!</p>
                <a href="/chef/login" style="color: #3498db;">← Попробовать снова</a>
            </body>
            </html>
            '''
    
    # GET запрос - показываем форму входа
    return '''
    <!DOCTYPE html>
    <html>
    <head>
        <title>Вход для повара</title>
        <meta charset="utf-8">
        <style>
            body { 
                font-family: 'Arial', sans-serif; 
                background: linear-gradient(135deg, #f39c12 0%, #e67e22 100%);
                min-height: 100vh;
                display: flex;
                justify-content: center;
                align-items: center;
                padding: 20px;
            }
            .container {
                background: white;
                padding: 40px;
                border-radius: 20px;
                box-shadow: 0 20px 60px rgba(0,0,0,0.3);
                max-width: 500px;
                width: 100%;
            }
            h1 {
                color: #2c3e50;
                text-align: center;
                margin-bottom: 30px;
            }
            .form-group {
                margin-bottom: 20px;
            }
            label {
                display: block;
                margin-bottom: 8px;
                color: #2c3e50;
                font-weight: bold;
            }
            input {
                width: 100%;
                padding: 12px;
                border: 2px solid #ddd;
                border-radius: 8px;
                font-size: 1em;
                transition: border-color 0.3s;
            }
            input:focus {
                border-color: #e67e22;
                outline: none;
            }
            .btn {
                width: 100%;
                padding: 15px;
                border: none;
                border-radius: 10px;
                font-size: 1.1em;
                cursor: pointer;
                transition: all 0.3s;
                margin-top: 10px;
            }
            .btn-login {
                background: #e67e22;
                color: white;
            }
            .btn-login:hover {
                background: #d35400;
            }
            .btn-back {
                background: #95a5a6;
                color: white;
            }
            .btn-back:hover {
                background: #7f8c8d;
            }
            .register-link {
                text-align: center;
                margin-top: 20px;
            }
        </style>
    </head>
    <body>
        <div class="container">
            <h1>🔐 Вход для повара</h1>
            
            <form method="POST" action="/chef/login">
                <div class="form-group">
                    <label for="email">Email:</label>
                    <input type="email" id="email" name="email" required placeholder="Введите ваш email">
                </div>
                
                <div class="form-group">
                    <label for="password">Пароль:</label>
                    <input type="password" id="password" name="password" required placeholder="Введите пароль">
                </div>
                
                <button type="submit" class="btn btn-login">Войти</button>
                <a href="/chef" class="btn btn-back" style="text-decoration: none; display: block; text-align: center;">← Назад</a>
            </form>
            
            <div class="register-link">
                <p>Нет аккаунта? <a href="/chef/register">Зарегистрируйтесь</a></p>
            </div>
        </div>
    </body>
    </html>
    '''

# ==================== СТРАНИЦА АДМИНИСТРАТОРА ====================

@app.route('/admin')
def admin_page():
    """Страница выбора: вход или регистрация для администратора"""
    return '''
    <!DOCTYPE html>
    <html>
    <head>
        <title>Вход для администратора</title>
        <meta charset="utf-8">
        <style>
            body { 
                font-family: 'Arial', sans-serif; 
                background: linear-gradient(135deg, #2c3e50 0%, #34495e 100%);
                min-height: 100vh;
                display: flex;
                justify-content: center;
                align-items: center;
                padding: 20px;
            }
            .container {
                background: white;
                padding: 40px;
                border-radius: 20px;
                box-shadow: 0 20px 60px rgba(0,0,0,0.3);
                max-width: 500px;
                width: 100%;
            }
            h1 {
                color: #2c3e50;
                text-align: center;
                margin-bottom: 30px;
            }
            .btn-group {
                display: flex;
                flex-direction: column;
                gap: 15px;
            }
            .btn {
                padding: 15px;
                border: none;
                border-radius: 10px;
                font-size: 1.1em;
                cursor: pointer;
                transition: all 0.3s;
                text-align: center;
                text-decoration: none;
                display: block;
            }
            .btn-login {
                background: #2c3e50;
                color: white;
            }
            .btn-login:hover {
                background: #1a252f;
            }
            .btn-register {
                background: #34495e;
                color: white;
            }
            .btn-register:hover {
                background: #2c3e50;
            }
            .btn-back {
                background: #95a5a6;
                color: white;
                margin-top: 20px;
            }
            .btn-back:hover {
                background: #7f8c8d;
            }
        </style>
    </head>
    <body>
        <div class="container">
            <h1>👨‍💼 Вход для администратора</h1>
            <p style="text-align: center; margin-bottom: 30px; color: #7f8c8d;">
                Выберите действие:
            </p>
            
            <div class="btn-group">
                <a href="/admin/login" class="btn btn-login">Войти в аккаунт</a>
                <a href="/admin/register" class="btn btn-register">Зарегистрироваться</a>
                <a href="/" class="btn btn-back">← На главную</a>
            </div>
        </div>
    </body>
    </html>
    '''

# ==================== РЕГИСТРАЦИЯ АДМИНИСТРАТОРА ====================

@app.route('/admin/register', methods=['GET', 'POST'])
def admin_register():
    """Регистрация нового администратора с номером телефона"""
    if request.method == 'POST':
        # Получаем данные из формы
        email = request.form.get('email', '').strip()
        password = request.form.get('password', '').strip()
        confirm_password = request.form.get('confirm_password', '').strip()
        first_name = request.form.get('first_name', '').strip()
        last_name = request.form.get('last_name', '').strip()
        phone = request.form.get('phone', '').strip()
        position = request.form.get('position', '').strip()
        
        # Проверяем обязательные поля
        if not all([email, password, confirm_password, first_name, last_name, phone, position]):
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Все поля обязательны для заполнения!</p>
                <a href="/admin/register" style="color: #3498db;">← Вернуться к регистрации</a>
            </body>
            </html>
            '''
        
        # Проверяем совпадение паролей
        if password != confirm_password:
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Пароли не совпадают!</p>
                <a href="/admin/register" style="color: #3498db;">← Вернуться к регистрации</a>
            </body>
            </html>
            '''
        
        # Проверяем email
        if '@' not in email or '.' not in email:
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Введите корректный email адрес!</p>
                <a href="/admin/register" style="color: #3498db;">← Вернуться к регистрации</a>
            </body>
            </html>
            '''
        
        # Проверяем номер телефона
        if not validate_phone(phone):
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Введите корректный номер телефона!</p>
                <p>Формат: +7XXXXXXXXXX или 8XXXXXXXXXX</p>
                <a href="/admin/register" style="color: #3498db;">← Вернуться к регистрации</a>
            </body>
            </html>
            '''
        
        # Форматируем телефон
        formatted_phone = format_phone(phone)
        
        db = get_db()
        
        # Проверяем, нет ли уже такого email
        existing_user = db.execute('SELECT id FROM users WHERE email = ?', (email,)).fetchone()
        if existing_user:
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Пользователь с таким email уже зарегистрирован!</p>
                <a href="/admin/register" style="color: #3498db;">← Вернуться к регистрации</a>
            </body>
            </html>
            '''
        
        # Проверяем, нет ли уже такого телефона
        existing_phone = db.execute('SELECT id FROM users WHERE phone = ?', (formatted_phone,)).fetchone()
        if existing_phone:
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Пользователь с таким номером телефона уже зарегистрирован!</p>
                <a href="/admin/register" style="color: #3498db;">← Вернуться к регистрации</a>
            </body>
            </html>
            '''
        
        # Создаем нового администратора
        user_id = str(uuid.uuid4())
        hashed_password = generate_password_hash(password)
        
        db.execute('''
            INSERT INTO users (id, email, password, first_name, last_name, phone, role, position)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (user_id, email, hashed_password, first_name, last_name, formatted_phone, 'admin', position))
        
        db.commit()
        
        # Автоматически входим после регистрации
        session['user_id'] = user_id
        session['user_email'] = email
        session['user_name'] = f"{first_name} {last_name}"
        session['user_role'] = 'admin'
        session['user_position'] = position
        
        return redirect('/dashboard')
    
    # GET запрос - показываем форму регистрации
    return '''
    <!DOCTYPE html>
    <html>
    <head>
        <title>Регистрация администратора</title>
        <meta charset="utf-8">
        <style>
            body { 
                font-family: 'Arial', sans-serif; 
                background: linear-gradient(135deg, #2c3e50 0%, #34495e 100%);
                min-height: 100vh;
                display: flex;
                justify-content: center;
                align-items: center;
                padding: 20px;
            }
            .container {
                background: white;
                padding: 40px;
                border-radius: 20px;
                box-shadow: 0 20px 60px rgba(0,0,0,0.3);
                max-width: 500px;
                width: 100%;
            }
            h1 {
                color: #2c3e50;
                text-align: center;
                margin-bottom: 30px;
            }
            .form-group {
                margin-bottom: 20px;
            }
            label {
                display: block;
                margin-bottom: 8px;
                color: #2c3e50;
                font-weight: bold;
            }
            input, select {
                width: 100%;
                padding: 12px;
                border: 2px solid #ddd;
                border-radius: 8px;
                font-size: 1em;
                transition: border-color 0.3s;
            }
            input:focus, select:focus {
                border-color: #2c3e50;
                outline: none;
            }
            .help-text {
                font-size: 0.85em;
                color: #7f8c8d;
                margin-top: 5px;
            }
            .btn {
                width: 100%;
                padding: 15px;
                border: none;
                border-radius: 10px;
                font-size: 1.1em;
                cursor: pointer;
                transition: all 0.3s;
                margin-top: 10px;
            }
            .btn-register {
                background: #34495e;
                color: white;
            }
            .btn-register:hover {
                background: #2c3e50;
            }
            .btn-back {
                background: #95a5a6;
                color: white;
            }
            .btn-back:hover {
                background: #7f8c8d;
            }
        </style>
    </head>
    <body>
        <div class="container">
            <h1>📝 Регистрация администратора</h1>
            
            <form method="POST" action="/admin/register">
                <div class="form-group">
                    <label for="email">Email:</label>
                    <input type="email" id="email" name="email" required placeholder="admin@school.ru">
                </div>
                
                <div class="form-group">
                    <label for="password">Пароль:</label>
                    <input type="password" id="password" name="password" required minlength="6" placeholder="Не менее 6 символов">
                </div>
                
                <div class="form-group">
                    <label for="confirm_password">Повторите пароль:</label>
                    <input type="password" id="confirm_password" name="confirm_password" required placeholder="Повторите пароль">
                </div>
                
                <div class="form-group">
                    <label for="first_name">Имя:</label>
                    <input type="text" id="first_name" name="first_name" required placeholder="Алексей">
                </div>
                
                <div class="form-group">
                    <label for="last_name">Фамилия:</label>
                    <input type="text" id="last_name" name="last_name" required placeholder="Администраторов">
                </div>
                
                <div class="form-group">
                    <label for="phone">Номер телефона:</label>
                    <input type="tel" id="phone" name="phone" required placeholder="+7XXXXXXXXXX или 8XXXXXXXXXX">
                    <div class="help-text">Формат: +7XXX XXX XX XX или 8XXX XXX XX XX</div>
                </div>
                
                <div class="form-group">
                    <label for="position">Должность:</label>
                    <select id="position" name="position" required>
                        <option value="">Выберите должность</option>
                        <option value="Директор школы">Директор школы</option>
                        <option value="Завуч">Завуч</option>
                        <option value="Заведующий столовой">Заведующий столовой</option>
                        <option value="Бухгалтер">Бухгалтер</option>
                        <option value="Системный администратор">Системный администратор</option>
                    </select>
                </div>
                
                <button type="submit" class="btn btn-register">Зарегистрироваться</button>
                <a href="/admin" class="btn btn-back" style="text-decoration: none; display: block; text-align: center;">← Назад</a>
            </form>
        </div>
    </body>
    </html>
    '''

# ==================== ВХОД АДМИНИСТРАТОРА ====================

@app.route('/admin/login', methods=['GET', 'POST'])
def admin_login():
    """Вход для администратора"""
    if request.method == 'POST':
        email = request.form.get('email', '').strip()
        password = request.form.get('password', '').strip()
        
        if not email or not password:
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Заполните все поля!</p>
                <a href="/admin/login" style="color: #3498db;">← Вернуться ко входу</a>
            </body>
            </html>
            '''
        
        db = get_db()
        user = db.execute('SELECT * FROM users WHERE email = ? AND role = ?', (email, 'admin')).fetchone()
        
        if user and check_password_hash(user['password'], password):
            session['user_id'] = user['id']
            session['user_email'] = email
            session['user_name'] = f"{user['first_name']} {user['last_name']}"
            session['user_role'] = user['role']
            session['user_position'] = user['position']
            return redirect('/dashboard')
        else:
            return '''
            <!DOCTYPE html>
            <html>
            <head><title>Ошибка</title></head>
            <body style="font-family: Arial; text-align: center; padding: 50px;">
                <h2 style="color: #e74c3c;">❌ Ошибка</h2>
                <p>Неверный email или пароль!</p>
                <a href="/admin/login" style="color: #3498db;">← Попробовать снова</a>
            </body>
            </html>
            '''
    
    # GET запрос - показываем форму входа
    return '''
    <!DOCTYPE html>
    <html>
    <head>
        <title>Вход для администратора</title>
        <meta charset="utf-8">
        <style>
            body { 
                font-family: 'Arial', sans-serif; 
                background: linear-gradient(135deg, #2c3e50 0%, #34495e 100%);
                min-height: 100vh;
                display: flex;
                justify-content: center;
                align-items: center;
                padding: 20px;
            }
            .container {
                background: white;
                padding: 40px;
                border-radius: 20px;
                box-shadow: 0 20px 60px rgba(0,0,0,0.3);
                max-width: 500px;
                width: 100%;
            }
            h1 {
                color: #2c3e50;
                text-align: center;
                margin-bottom: 30px;
            }
            .form-group {
                margin-bottom: 20px;
            }
            label {
                display: block;
                margin-bottom: 8px;
                color: #2c3e50;
                font-weight: bold;
            }
            input {
                width: 100%;
                padding: 12px;
                border: 2px solid #ddd;
                border-radius: 8px;
                font-size: 1em;
                transition: border-color 0.3s;
            }
            input:focus {
                border-color: #2c3e50;
                outline: none;
            }
            .btn {
                width: 100%;
                padding: 15px;
                border: none;
                border-radius: 10px;
                font-size: 1.1em;
                cursor: pointer;
                transition: all 0.3s;
                margin-top: 10px;
            }
            .btn-login {
                background: #2c3e50;
                color: white;
            }
            .btn-login:hover {
                background: #1a252f;
            }
            .btn-back {
                background: #95a5a6;
                color: white;
            }
            .btn-back:hover {
                background: #7f8c8d;
            }
            .register-link {
                text-align: center;
                margin-top: 20px;
            }
        </style>
    </head>
    <body>
        <div class="container">
            <h1>🔐 Вход для администратора</h1>
            
            <form method="POST" action="/admin/login">
                <div class="form-group">
                    <label for="email">Email:</label>
                    <input type="email" id="email" name="email" required placeholder="Введите ваш email">
                </div>
                
                <div class="form-group">
                    <label for="password">Пароль:</label>
                    <input type="password" id="password" name="password" required placeholder="Введите пароль">
                </div>
                
                <button type="submit" class="btn btn-login">Войти</button>
                <a href="/admin" class="btn btn-back" style="text-decoration: none; display: block; text-align: center;">← Назад</a>
            </form>
            
            <div class="register-link">
                <p>Нет аккаунта? <a href="/admin/register">Зарегистрируйтесь</a></p>
            </div>
        </div>
    </body>
    </html>
    '''

# ==================== ПАНЕЛЬ УПРАВЛЕНИЯ ====================

@app.route('/dashboard')
def dashboard():
    """Панель управления после входа"""
    if 'user_id' not in session:
        return redirect('/')
    
    role = session.get('user_role')
    
    if role == 'student':
        return student_dashboard()
    elif role == 'chef':
        return chef_dashboard()
    elif role == 'admin':
        return admin_dashboard()
    
    return redirect('/')

def student_dashboard():
    """Панель ученика"""
    db = get_db()
    today = date.today()
    today_str = today.strftime('%Y-%m-%d')
    
    # Получаем информацию о пользователе
    user = db.execute('SELECT * FROM users WHERE id = ?', (session.get('user_id'),)).fetchone()
    if not user:
        session.clear()
        return redirect('/')
    
    # Форматируем телефон для отображения
    phone_display = user['phone'] if user['phone'] else 'Не указан'
    
    # Получаем меню на сегодня
    menu = db.execute('SELECT * FROM menu WHERE day = ? AND available = 1', (today_str,)).fetchall()
    
    # Получаем активные абонементы пользователя
    subscriptions = db.execute('''
        SELECT s.*, m.name as menu_name, m.price, m.meal_type
        FROM subscriptions s
        JOIN menu m ON s.menu_id = m.id
        WHERE s.user_id = ? AND s.status = 'активен'
        ORDER BY s.created_at DESC
    ''', (session.get('user_id'),)).fetchall()
    
    # Получаем выданные сегодня блюда
    issued_today = db.execute('''
        SELECT im.*, m.name as menu_name
        FROM issued_meals im
        JOIN menu m ON im.menu_id = m.id
        WHERE im.user_id = ? AND im.issue_date = ? 
        ORDER BY im.created_at DESC
    ''', (session.get('user_id'), today_str)).fetchall()
    
    # Получаем отзывы пользователя
    user_reviews = db.execute('''
        SELECT r.*, m.name as menu_name
        FROM reviews r
        JOIN menu m ON r.menu_id = m.id
        WHERE r.user_id = ?
        ORDER BY r.created_at DESC
        LIMIT 5
    ''', (session.get('user_id'),)).fetchall()
    
    html = f'''
    <!DOCTYPE html>
    <html>
    <head>
        <title>Панель ученика</title>
        <meta charset="utf-8">
        <style>
            body {{ 
                font-family: 'Arial', sans-serif; 
                background: #f0f8ff;
                margin: 0;
                padding: 20px;
            }}
            .header {{ 
                background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
                color: white; 
                padding: 30px;
                border-radius: 15px;
                margin-bottom: 30px;
                box-shadow: 0 10px 30px rgba(0,0,0,0.2);
            }}
            .dashboard-grid {{
                display: grid;
                grid-template-columns: 1fr 1fr;
                gap: 30px;
                margin-bottom: 30px;
            }}
            .section {{
                background: white;
                padding: 25px;
                border-radius: 15px;
                box-shadow: 0 5px 15px rgba(0,0,0,0.1);
            }}
            .menu-grid {{ 
                display: grid; 
                grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); 
                gap: 25px;
                margin-top: 30px;
            }}
            .menu-card {{ 
                background: white;
                padding: 25px;
                border-radius: 15px;
                box-shadow: 0 5px 15px rgba(0,0,0,0.1);
                transition: transform 0.3s;
                border-left: 5px solid #3498db;
            }}
            .menu-card:hover {{
                transform: translateY(-5px);
                box-shadow: 0 10px 25px rgba(0,0,0,0.15);
            }}
            .btn {{ 
                background: #27ae60; 
                color: white; 
                padding: 12px 25px;
                border: none; 
                border-radius: 8px; 
                cursor: pointer; 
                margin-top: 15px;
                font-size: 1em;
                transition: background 0.3s;
                display: inline-block;
                text-decoration: none;
            }}
            .btn:hover {{ 
                background: #219653; 
            }}
            .btn-subscription {{
                background: #3498db;
                margin-left: 10px;
            }}
            .btn-subscription:hover {{
                background: #2980b9;
            }}
            .btn-review {{
                background: #f39c12;
                margin-left: 10px;
            }}
            .btn-review:hover {{
                background: #e67e22;
            }}
            .balance {{ 
                font-size: 1.8em; 
                color: #2ecc71; 
                font-weight: bold; 
                background: rgba(255, 255, 255, 0.2);
                padding: 10px 20px;
                border-radius: 10px;
                display: inline-block;
                margin: 10px 0;
            }}
            .logout-btn {{
                background: #e74c3c;
                color: white;
                padding: 10px 20px;
                border: none;
                border-radius: 8px;
                cursor: pointer;
                text-decoration: none;
                display: inline-block;
                margin-left: 20px;
                transition: background 0.3s;
            }}
            .logout-btn:hover {{ 
                background: #c0392b; 
            }}
            .user-info {{
                margin-bottom: 15px;
            }}
            .phone-info {{
                background: rgba(255, 255, 255, 0.2);
                padding: 8px 15px;
                border-radius: 8px;
                display: inline-block;
                margin: 10px 0;
            }}
            .meal-type {{
                display: inline-block;
                background: #3498db;
                color: white;
                padding: 5px 15px;
                border-radius: 20px;
                font-size: 0.9em;
                margin-right: 10px;
                margin-bottom: 10px;
            }}
            .subscription-card {{
                background: #e8f4fc;
                padding: 15px;
                border-radius: 10px;
                margin: 10px 0;
                border-left: 4px solid #3498db;
            }}
            .issued-card {{
                background: #e8f6e8;
                padding: 15px;
                border-radius: 10px;
                margin: 10px 0;
                border-left: 4px solid #27ae60;
            }}
            .status-badge {{
                display: inline-block;
                padding: 5px 10px;
                border-radius: 15px;
                font-size: 0.8em;
                margin-left: 10px;
            }}
            .status-waiting {{ background: #f39c12; color: white; }}
            .status-issued {{ background: #27ae60; color: white; }}
            
            /* Календарь */
            .calendar-container {{
                display: none;
                position: fixed;
                top: 0;
                left: 0;
                width: 100%;
                height: 100%;
                background: rgba(0,0,0,0.7);
                z-index: 1000;
                justify-content: center;
                align-items: center;
            }}
            .calendar {{
                background: white;
                padding: 30px;
                border-radius: 15px;
                width: 90%;
                max-width: 800px;
                max-height: 90vh;
                overflow-y: auto;
            }}
            .calendar-header {{
                display: flex;
                justify-content: space-between;
                align-items: center;
                margin-bottom: 20px;
            }}
            .calendar-grid {{
                display: grid;
                grid-template-columns: repeat(7, 1fr);
                gap: 5px;
                text-align: center;
            }}
            .calendar-day {{
                padding: 10px;
                border: 1px solid #ddd;
                border-radius: 5px;
                cursor: pointer;
                transition: all 0.3s;
            }}
            .calendar-day:hover {{
                background: #f0f8ff;
            }}
            .calendar-day.selected {{
                background: #3498db;
                color: white;
                border-color: #3498db;
            }}
            .calendar-day.today {{
                border: 2px solid #e74c3c;
            }}
            .calendar-day.weekend {{
                background: #f8f9fa;
            }}
            .calendar-day-header {{
                font-weight: bold;
                background: #f8f9fa;
                padding: 10px;
            }}
            .instructions {{
                background: #f8f9fa;
                padding: 15px;
                border-radius: 10px;
                margin: 20px 0;
                border-left: 4px solid #3498db;
            }}
            .selected-dates {{
                background: #e8f4fc;
                padding: 15px;
                border-radius: 10px;
                margin: 20px 0;
                min-height: 50px;
            }}
            .date-chip {{
                display: inline-block;
                background: #3498db;
                color: white;
                padding: 5px 10px;
                border-radius: 15px;
                margin: 5px;
                font-size: 0.9em;
            }}
            .date-chip button {{
                background: none;
                border: none;
                color: white;
                margin-left: 5px;
                cursor: pointer;
            }}
            .star-rating {{
                color: #f39c12;
                font-size: 1.2em;
                margin: 10px 0;
            }}
        </style>
    </head>
    <body>
        <div class="header">
            <h1>👨‍🎓 Добро пожаловать, {user['first_name']} {user['last_name']}!</h1>
            <div class="user-info">
                <p><strong>Класс:</strong> {user['class_name']} | <strong>Баланс:</strong> <span class="balance" id="userBalance">{user['balance']} руб.</span></p>
                <div class="phone-info">
                    📱 <strong>Телефон:</strong> {phone_display}
                </div>
            </div>
            <a href="/logout" class="logout-btn">Выйти</a>
        </div>
        
        <div class="dashboard-grid">
            <div class="section">
                <h2>📋 Ваши абонементы:</h2>
    '''
    
    if subscriptions:
        for sub in subscriptions:
            selected_dates = json.loads(sub['selected_dates'])
            html += f'''
                <div class="subscription-card">
                    <h4>{sub['menu_name']} ({sub['meal_type']})</h4>
                    <p><strong>Период:</strong> {sub['start_date']} - {sub['end_date']}</p>
                    <p><strong>Выбрано дней:</strong> {len(selected_dates)}</p>
                    <p><strong>Стоимость:</strong> {sub['total_price']} руб.</p>
                    <p><strong>Статус:</strong> <span class="status-badge status-issued">{sub['status']}</span></p>
                </div>
            '''
    else:
        html += '<p>У вас нет активных абонементов.</p>'
    
    html += '''
            </div>
            
            <div class="section">
                <h2>🍽️ Получено сегодня:</h2>
    '''
    
    if issued_today:
        for meal in issued_today:
            status_class = 'status-issued' if meal['status'] == 'выдан' else 'status-waiting'
            html += f'''
                <div class="issued-card">
                    <h4>{meal['menu_name']}</h4>
                    <p><strong>Время выдачи:</strong> {meal['created_at'][11:16] if meal['created_at'] else 'не указано'}</p>
                    <p><strong>Статус:</strong> <span class="status-badge {status_class}">{meal['status']}</span></p>
                </div>
            '''
    else:
        html += '<p>Сегодня вы еще не получали питание.</p>'
    
    html += f'''
            </div>
        </div>
        
        <h2>🍽️ Меню на сегодня ({today_str}):</h2>
        
        <div class="menu-grid">
    '''
    
    if menu:
        for item in menu:
            # Получаем рейтинг для каждого блюда
            avg_rating = db.execute('''
                SELECT AVG(rating) as avg_rating, COUNT(*) as review_count 
                FROM reviews 
                WHERE menu_id = ?
            ''', (item['id'],)).fetchone()
            
            avg_rating_value = avg_rating['avg_rating'] if avg_rating['avg_rating'] else 0
            review_count = avg_rating['review_count'] or 0
            
            # Проверяем, есть ли у пользователя абонемент на это блюдо сегодня
            has_subscription_today = False
            for sub in subscriptions:
                selected_dates = json.loads(sub['selected_dates'])
                if sub['menu_id'] == item['id'] and today_str in selected_dates:
                    has_subscription_today = True
                    break
            
            html += f'''
            <div class="menu-card">
                <h3>{item['name']}</h3>
                <p>{item['description']}</p>
                <span class="meal-type">{item['meal_type']}</span>
                <p><strong>Цена:</strong> {item['price']} руб.</p>
                <div class="star-rating">
                    {'★' * int(avg_rating_value)}{'☆' * (5 - int(avg_rating_value))}
                    <span style="font-size: 0.9em; color: #7f8c8d;">
                        ({avg_rating_value:.1f} из 5, {review_count} отзывов)
                    </span>
                </div>
                <div>
                    <button onclick="orderItem('{item['id']}')" class="btn">Заказать на сегодня</button>
            '''
            
            if not has_subscription_today:
                html += f'''<button onclick="openSubscriptionForm('{item['id']}', '{item['name']}', {item['price']}, '{item['meal_type']}')" class="btn btn-subscription">Оформить абонемент</button>'''
            else:
                html += '''<button class="btn" style="background: #95a5a6;" disabled>Абонемент оформлен</button>'''
            
            html += f'''
                    <button onclick="openReviewForm('{item['id']}', '{item['name']}')" class="btn btn-review">Оставить отзыв</button>
                </div>
            </div>
            '''
    else:
        html += '<p>На сегодня меню нет.</p>'
    
    html += '''
        </div>
        
        <!-- Модальное окно для оформления абонемента -->
        <div id="subscriptionModal" class="calendar-container">
            <div class="calendar">
                <div class="calendar-header">
                    <h2 id="subscriptionTitle">Оформление абонемента</h2>
                    <button onclick="closeSubscriptionForm()" style="background: #e74c3c; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer;">✕ Закрыть</button>
                </div>
                
                <div class="instructions">
                    <h3>🎯 Как пользоваться календарем:</h3>
                    <p>1. <strong>Выберите даты</strong> - кликайте на дни в календаре, когда хотите получать это блюдо</p>
                    <p>2. <strong>Можно выбрать несколько дат</strong> - например, только учебные дни (пн-пт)</p>
                    <p>3. <strong>Цена пересчитывается автоматически</strong> - чем больше дней, тем выше общая стоимость</p>
                    <p>4. <strong>Отменить выбор</strong> - кликните на выбранную дату еще раз, чтобы убрать ее</p>
                    <p>5. <strong>Подтвердить выбор</strong> - нажмите кнопку "Оформить абонемент" внизу</p>
                </div>
                
                <h3>📅 Выберите даты:</h3>
                <div class="calendar-grid">
                    <div class="calendar-day-header">Пн</div>
                    <div class="calendar-day-header">Вт</div>
                    <div class="calendar-day-header">Ср</div>
                    <div class="calendar-day-header">Чт</div>
                    <div class="calendar-day-header">Пт</div>
                    <div class="calendar-day-header" style="color: #e74c3c;">Сб</div>
                    <div class="calendar-day-header" style="color: #e74c3c;">Вс</div>
    '''
    
    # Генерируем календарь на 42 дня (6 недель)
    for i in range(42):
        day_date = today + timedelta(days=i)
        date_str = day_date.strftime('%Y-%m-%d')
        day_name = ['Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб', 'Вс'][day_date.weekday()]
        is_weekend = day_date.weekday() >= 5
        is_today = i == 0
        
        day_class = 'calendar-day'
        if is_today:
            day_class += ' today'
        if is_weekend:
            day_class += ' weekend'
        
        html += f'''
            <div class="{day_class}" data-date="{date_str}" onclick="toggleDate('{date_str}')">
                {day_date.day}
            </div>
        '''
    
    html += '''
                </div>
                
                <div class="selected-dates">
                    <h4>📋 Выбранные даты:</h4>
                    <div id="selectedDatesList"></div>
                </div>
                
                <div style="margin-top: 20px;">
                    <p><strong>Цена за один день:</strong> <span id="dayPrice">0</span> руб.</p>
                    <p><strong>Выбрано дней:</strong> <span id="selectedDaysCount">0</span></p>
                    <p><strong>Общая стоимость:</strong> <span id="totalPrice" style="font-size: 1.5em; color: #27ae60; font-weight: bold;">0</span> руб.</p>
                </div>
                
                <input type="hidden" id="selectedMenuId" value="">
                <input type="hidden" id="selectedMenuPrice" value="0">
                
                <div style="margin-top: 30px;">
                    <button onclick="createSubscription()" class="btn" style="font-size: 1.2em; padding: 15px 30px;">✅ Оформить абонемент</button>
                    <button onclick="closeSubscriptionForm()" class="btn" style="background: #95a5a6; margin-left: 10px;">❌ Отмена</button>
                </div>
            </div>
        </div>
        
        <!-- Модальное окно для отзывов -->
        <div id="reviewModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 1000; justify-content: center; align-items: center;">
            <div style="background: white; padding: 30px; border-radius: 15px; width: 90%; max-width: 500px;">
                <h3 id="reviewTitle">Оставить отзыв</h3>
                <div id="ratingStars" class="rating-stars">
                    <span data-rating="1">☆</span>
                    <span data-rating="2">☆</span>
                    <span data-rating="3">☆</span>
                    <span data-rating="4">☆</span>
                    <span data-rating="5">☆</span>
                </div>
                <input type="hidden" id="selectedRating" value="0">
                <input type="hidden" id="selectedMenuIdReview" value="">
                <input type="hidden" id="editReviewId" value="">
                <textarea id="reviewComment" placeholder="Напишите ваш отзыв здесь..." style="width: 100%; height: 100px; padding: 10px; border: 1px solid #ddd; border-radius: 5px; margin-top: 10px;"></textarea>
                <div style="margin-top: 20px;">
                    <button onclick="submitReview()" class="btn" style="margin-right: 10px;">Отправить отзыв</button>
                    <button onclick="closeReviewForm()" class="btn" style="background: #95a5a6;">Отмена</button>
                </div>
            </div>
        </div>
        
        <div id="message" style="margin-top: 40px; padding: 25px; background: white; border-radius: 15px; box-shadow: 0 5px 15px rgba(0,0,0,0.1);"></div>
        
        <script>
            let selectedDates = new Set();
            let currentRating = 0;
            let dayPrice = 0;
            
            function orderItem(menuId) {
                fetch('/api/order', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({menu_id: menuId})
                })
                .then(response => response.json())
                .then(data => {
                    if (data.error) {
                        showMessage('error', data.error);
                    } else {
                        showMessage('success', '✅ <strong>Заказ успешно создан!</strong><br><br>📋 <strong>Номер заказа:</strong> ' + data.order_id + '<br>💰 <strong>Новый баланс:</strong> ' + data.new_balance + ' руб.');
                        // Обновляем баланс на странице
                        document.getElementById('userBalance').textContent = data.new_balance + ' руб.';
                    }
                })
                .catch(error => {
                    showMessage('error', '❌ Ошибка: ' + error);
                });
            }
            
            function openSubscriptionForm(menuId, menuName, price, mealType) {
                document.getElementById('subscriptionTitle').textContent = 'Абонемент: ' + menuName + ' (' + mealType + ')';
                document.getElementById('selectedMenuId').value = menuId;
                document.getElementById('selectedMenuPrice').value = price;
                document.getElementById('dayPrice').textContent = price;
                
                // Сброс выбранных дат
                selectedDates.clear();
                updateSelectedDatesList();
                updatePrice();
                
                // Открытие модального окна
                document.getElementById('subscriptionModal').style.display = 'flex';
                dayPrice = price;
            }
            
            function closeSubscriptionForm() {
                document.getElementById('subscriptionModal').style.display = 'none';
            }
            
            function toggleDate(date) {
                if (selectedDates.has(date)) {
                    selectedDates.delete(date);
                } else {
                    selectedDates.add(date);
                }
                
                // Обновляем отображение
                updateCalendarSelection();
                updateSelectedDatesList();
                updatePrice();
            }
            
            function updateCalendarSelection() {
                document.querySelectorAll('.calendar-day').forEach(day => {
                    const date = day.getAttribute('data-date');
                    if (selectedDates.has(date)) {
                        day.classList.add('selected');
                    } else {
                        day.classList.remove('selected');
                    }
                });
            }
            
            function updateSelectedDatesList() {
                const container = document.getElementById('selectedDatesList');
                container.innerHTML = '';
                
                const sortedDates = Array.from(selectedDates).sort();
                
                sortedDates.forEach(date => {
                    const dateObj = new Date(date);
                    const dateStr = dateObj.toLocaleDateString('ru-RU', {
                        weekday: 'short',
                        day: 'numeric',
                        month: 'short'
                    });
                    
                    const chip = document.createElement('div');
                    chip.className = 'date-chip';
                    chip.innerHTML = `
                        ${dateStr}
                        <button onclick="removeDate('${date}')">✕</button>
                    `;
                    container.appendChild(chip);
                });
                
                document.getElementById('selectedDaysCount').textContent = selectedDates.size;
            }
            
            function removeDate(date) {
                selectedDates.delete(date);
                updateCalendarSelection();
                updateSelectedDatesList();
                updatePrice();
            }
            
            function updatePrice() {
                const totalPrice = dayPrice * selectedDates.size;
                document.getElementById('totalPrice').textContent = totalPrice;
            }
            
            function createSubscription() {
                const menuId = document.getElementById('selectedMenuId').value;
                const dates = Array.from(selectedDates);
                
                if (dates.length === 0) {
                    showMessage('error', '❌ Выберите хотя бы одну дату для абонемента');
                    return;
                }
                
                const sortedDates = dates.sort();
                const startDate = sortedDates[0];
                const endDate = sortedDates[sortedDates.length - 1];
                const totalPrice = dayPrice * dates.length;
                
                fetch('/api/subscription', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({
                        menu_id: menuId,
                        selected_dates: dates,
                        start_date: startDate,
                        end_date: endDate,
                        total_price: totalPrice
                    })
                })
                .then(response => response.json())
                .then(data => {
                    if (data.error) {
                        showMessage('error', data.error);
                    } else {
                        showMessage('success', '✅ <strong>Абонемент успешно оформлен!</strong><br><br>' +
                            '📋 <strong>Номер абонемента:</strong> ' + data.subscription_id + '<br>' +
                            '📅 <strong>Количество дней:</strong> ' + data.days_count + '<br>' +
                            '💰 <strong>Стоимость:</strong> ' + data.total_price + ' руб.<br>' +
                            '💳 <strong>Новый баланс:</strong> ' + data.new_balance + ' руб.');
                        
                        // Обновляем баланс на странице
                        document.getElementById('userBalance').textContent = data.new_balance + ' руб.';
                        
                        // Закрываем модальное окно и перезагружаем страницу
                        closeSubscriptionForm();
                        setTimeout(() => location.reload(), 3000);
                    }
                })
                .catch(error => {
                    showMessage('error', '❌ Ошибка: ' + error);
                });
            }
            
            // Функции для отзывов
            function openReviewForm(menuId, menuName) {
                document.getElementById('reviewTitle').textContent = 'Оставить отзыв: ' + menuName;
                document.getElementById('selectedMenuIdReview').value = menuId;
                document.getElementById('editReviewId').value = '';
                document.getElementById('reviewComment').value = '';
                currentRating = 0;
                updateStars();
                document.getElementById('reviewModal').style.display = 'flex';
            }
            
            function closeReviewForm() {
                document.getElementById('reviewModal').style.display = 'none';
            }
            
            function updateStars() {
                const stars = document.querySelectorAll('#ratingStars span');
                stars.forEach((star, index) => {
                    if (index < currentRating) {
                        star.textContent = '★';
                        star.style.color = '#f39c12';
                    } else {
                        star.textContent = '☆';
                        star.style.color = '#ddd';
                    }
                });
                document.getElementById('selectedRating').value = currentRating;
            }
            
            function submitReview() {
                const menuId = document.getElementById('selectedMenuIdReview').value;
                const rating = currentRating;
                const comment = document.getElementById('reviewComment').value.trim();
                const reviewId = document.getElementById('editReviewId').value;
                
                if (rating === 0) {
                    showMessage('error', '❌ Пожалуйста, поставьте оценку');
                    return;
                }
                
                if (!comment) {
                    showMessage('error', '❌ Пожалуйста, напишите отзыв');
                    return;
                }
                
                const url = reviewId ? '/api/review/' + reviewId : '/api/review';
                const method = reviewId ? 'PUT' : 'POST';
                
                fetch(url, {
                    method: method,
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({
                        menu_id: menuId,
                        rating: rating,
                        comment: comment
                    })
                })
                .then(response => response.json())
                .then(data => {
                    if (data.error) {
                        showMessage('error', data.error);
                    } else {
                        showMessage('success', reviewId ? '✅ Отзыв обновлен!' : '✅ Отзыв успешно добавлен!');
                        closeReviewForm();
                        setTimeout(() => location.reload(), 2000);
                    }
                })
                .catch(error => {
                    showMessage('error', '❌ Ошибка: ' + error);
                });
            }
            
            // Назначаем обработчики для звезд
            document.addEventListener('DOMContentLoaded', function() {
                const stars = document.querySelectorAll('#ratingStars span');
                stars.forEach(star => {
                    star.addEventListener('click', function() {
                        currentRating = parseInt(this.getAttribute('data-rating'));
                        updateStars();
                    });
                    
                    star.addEventListener('mouseover', function() {
                        const hoverRating = parseInt(this.getAttribute('data-rating'));
                        const stars = document.querySelectorAll('#ratingStars span');
                        stars.forEach((s, index) => {
                            if (index < hoverRating) {
                                s.style.color = '#f39c12';
                            } else {
                                s.style.color = '#ddd';
                            }
                        });
                    });
                    
                    star.addEventListener('mouseout', function() {
                        updateStars();
                    });
                });
            });
            
            function showMessage(type, content) {
                const messageDiv = document.getElementById('message');
                if (type === 'error') {
                    messageDiv.innerHTML = '<div style="background: #f8d7da; color: #721c24; padding: 20px; border-radius: 10px; border-left: 5px solid #e74c3c;">' + content + '</div>';
                } else {
                    messageDiv.innerHTML = '<div style="background: #d4edda; color: #155724; padding: 20px; border-radius: 10px; border-left: 5px solid #27ae60;">' + content + '</div>';
                }
                messageDiv.scrollIntoView({behavior: 'smooth'});
            }
        </script>
    </body>
    </html>
    '''
    
    return html

def chef_dashboard():
    """Панель повара"""
    db = get_db()
    today = date.today().strftime('%Y-%m-%d')
    
    # Получаем информацию о пользователе
    user = db.execute('SELECT * FROM users WHERE id = ?', (session.get('user_id'),)).fetchone()
    if not user:
        session.clear()
        return redirect('/')
    
    # Форматируем телефон для отображения
    phone_display = user['phone'] if user['phone'] else 'Не указан'
    
    # Получаем заказы на сегодня
    orders = db.execute('''
        SELECT o.*, u.first_name, u.last_name, u.class_name, m.name as menu_name, m.price, m.meal_type
        FROM orders o
        JOIN users u ON o.user_id = u.id
        JOIN menu m ON o.menu_id = m.id
        WHERE DATE(o.created_at) = ?
        ORDER BY o.created_at DESC
    ''', (today,)).fetchall()
    
    # Получаем абонементы на сегодня
    subscriptions_today = db.execute('''
        SELECT s.*, u.first_name, u.last_name, u.class_name, m.name as menu_name, m.price, m.meal_type
        FROM subscriptions s
        JOIN users u ON s.user_id = u.id
        JOIN menu m ON s.menu_id = m.id
        WHERE s.status = 'активен' AND json_extract(s.selected_dates, '$') LIKE ?
        ORDER BY s.created_at DESC
    ''', (f'%{today}%',)).fetchall()
    
    # Получаем блюда для выдачи сегодня
    meals_to_issue = db.execute('''
        SELECT im.*, u.first_name, u.last_name, u.class_name, m.name as menu_name, m.meal_type
        FROM issued_meals im
        JOIN users u ON im.user_id = u.id
        JOIN menu m ON im.menu_id = m.id
        WHERE im.issue_date = ? AND im.status = 'ожидает выдачи'
        ORDER BY im.created_at ASC
    ''', (today,)).fetchall()
    
    # Получаем уже выданные сегодня блюда
    issued_meals = db.execute('''
        SELECT im.*, u.first_name, u.last_name, u.class_name, m.name as menu_name, m.meal_type
        FROM issued_meals im
        JOIN users u ON im.user_id = u.id
        JOIN menu m ON im.menu_id = m.id
        WHERE im.issue_date = ? AND im.status = 'выдан' AND im.issued_by = ?
        ORDER BY im.created_at DESC
        LIMIT 20
    ''', (today, session.get('user_id'))).fetchall()
    
    html = f'''
    <!DOCTYPE html>
    <html>
    <head>
        <title>Панель повара</title>
        <meta charset="utf-8">
        <style>
            body {{
                font-family: 'Arial', sans-serif;
                background: #fff8f0;
                margin: 0;
                padding: 20px;
            }}
            .header {{
                background: linear-gradient(135deg, #f39c12 0%, #e67e22 100%);
                color: white;
                padding: 30px;
                border-radius: 15px;
                margin-bottom: 30px;
                box-shadow: 0 10px 30px rgba(0,0,0,0.2);
            }}
            .dashboard-grid {{
                display: grid;
                grid-template-columns: repeat(auto-fit, minmax(300px, 1fr));
                gap: 30px;
                margin-bottom: 30px;
            }}
            .section {{
                background: white;
                padding: 25px;
                border-radius: 15px;
                box-shadow: 0 5px 15px rgba(0,0,0,0.1);
            }}
            .meal-card {{
                background: #f8f9fa;
                padding: 15px;
                border-radius: 10px;
                margin: 10px 0;
                border-left: 4px solid #e67e22;
            }}
            .meal-card.issued {{
                border-left-color: #27ae60;
                background: #e8f6e8;
            }}
            .status-badge {{
                display: inline-block;
                padding: 5px 10px;
                border-radius: 15px;
                font-size: 0.8em;
                margin: 5px;
            }}
            .status-waiting {{ background: #f39c12; color: white; }}
            .status-issued {{ background: #27ae60; color: white; }}
            .btn {{
                background: #e67e22;
                color: white;
                padding: 8px 16px;
                border: none;
                border-radius: 5px;
                cursor: pointer;
                margin: 5px;
                transition: background 0.3s;
            }}
            .btn:hover {{
                background: #d35400;
            }}
            .btn-issue {{
                background: #27ae60;
            }}
            .btn-issue:hover {{
                background: #219653;
            }}
            .logout-btn {{
                background: #e74c3c;
                color: white;
                padding: 10px 20px;
                border: none;
                border-radius: 8px;
                cursor: pointer;
                text-decoration: none;
                display: inline-block;
                margin-left: 20px;
                transition: background 0.3s;
            }}
            .logout-btn:hover {{
                background: #c0392b;
            }}
            .statistics {{
                display: grid;
                grid-template-columns: repeat(auto-fit, minmax(150px, 1fr));
                gap: 15px;
                margin-bottom: 20px;
            }}
            .stat-card {{
                background: #f8f9fa;
                padding: 15px;
                border-radius: 10px;
                text-align: center;
            }}
            .stat-value {{
                font-size: 1.5em;
                font-weight: bold;
                color: #e67e22;
                margin: 5px 0;
            }}
            .phone-info {{
                background: rgba(255, 255, 255, 0.2);
                padding: 8px 15px;
                border-radius: 8px;
                display: inline-block;
                margin: 10px 0;
            }}
        </style>
    </head>
    <body>
        <div class="header">
            <h1>👨‍🍳 Панель повара</h1>
            <p>Добро пожаловать, {user['first_name']} {user['last_name']}!</p>
            <div class="phone-info">
                📱 <strong>Телефон:</strong> {phone_display}
            </div>
            <p><strong>Должность:</strong> {user['position']} | <strong>Дата:</strong> {today}</p>
            <a href="/logout" class="logout-btn">Выйти</a>
        </div>
        
        <div class="statistics">
            <div class="stat-card">
                <div class="stat-value">{len(orders)}</div>
                <div>Разовых заказов</div>
            </div>
            <div class="stat-card">
                <div class="stat-value">{len(subscriptions_today)}</div>
                <div>Абонементов</div>
            </div>
            <div class="stat-card">
                <div class="stat-value">{len(meals_to_issue)}</div>
                <div>Ожидают выдачи</div>
            </div>
            <div class="stat-card">
                <div class="stat-value">{len(issued_meals)}</div>
                <div>Выдано сегодня</div>
            </div>
        </div>
        
        <div class="dashboard-grid">
            <div class="section">
                <h2>⏳ Ожидают выдачи:</h2>
    '''
    
    if meals_to_issue:
        for meal in meals_to_issue:
            html += f'''
                <div class="meal-card">
                    <h4>{meal['menu_name']} ({meal['meal_type']})</h4>
                    <p><strong>Ученик:</strong> {meal['first_name']} {meal['last_name']} ({meal['class_name']})</p>
                    <p><strong>Статус:</strong> <span class="status-badge status-waiting">{meal['status']}</span></p>
                    <button onclick="issueMeal('{meal['id']}')" class="btn btn-issue">✅ Выдать</button>
                </div>
            '''
    else:
        html += '<p>Нет блюд, ожидающих выдачи.</p>'
    
    html += '''
            </div>
            
            <div class="section">
                <h2>📋 Абонементы на сегодня:</h2>
    '''
    
    if subscriptions_today:
        for sub in subscriptions_today:
            selected_dates = json.loads(sub['selected_dates'])
            html += f'''
                <div class="meal-card">
                    <h4>{sub['menu_name']} ({sub['meal_type']})</h4>
                    <p><strong>Ученик:</strong> {sub['first_name']} {sub['last_name']} ({sub['class_name']})</p>
                    <p><strong>Абонемент до:</strong> {sub['end_date']}</p>
                    <button onclick="createIssueFromSubscription('{sub['id']}', '{sub['menu_id']}', '{sub['user_id']}')" class="btn btn-issue">✅ Выдать по абонементу</button>
                </div>
            '''
    else:
        html += '<p>На сегодня абонементов нет.</p>'
    
    html += '''
            </div>
            
            <div class="section">
                <h2>✅ Выданные сегодня вами:</h2>
    '''
    
    if issued_meals:
        for meal in issued_meals:
            html += f'''
                <div class="meal-card issued">
                    <h4>{meal['menu_name']} ({meal['meal_type']})</h4>
                    <p><strong>Ученик:</strong> {meal['first_name']} {meal['last_name']} ({meal['class_name']})</p>
                    <p><strong>Время выдачи:</strong> {meal['created_at'][11:16] if meal['created_at'] else 'не указано'}</p>
                    <p><strong>Статус:</strong> <span class="status-badge status-issued">{meal['status']}</span></p>
                </div>
            '''
    else:
        html += '<p>Вы еще не выдавали блюд сегодня.</p>'
    
    html += '''
            </div>
        </div>
        
        <div id="message" style="margin-top: 40px; padding: 25px; background: white; border-radius: 15px; box-shadow: 0 5px 15px rgba(0,0,0,0.1);"></div>
        
        <script>
            function issueMeal(mealId) {
                fetch('/api/issue-meal', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({meal_id: mealId})
                })
                .then(response => response.json())
                .then(data => {
                    if (data.error) {
                        showMessage('error', data.error);
                    } else {
                        showMessage('success', '✅ Блюдо успешно выдано!');
                        setTimeout(() => location.reload(), 1500);
                    }
                })
                .catch(error => {
                    showMessage('error', '❌ Ошибка: ' + error);
                });
            }
            
            function createIssueFromSubscription(subscriptionId, menuId, userId) {
                fetch('/api/issue-from-subscription', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({
                        subscription_id: subscriptionId,
                        menu_id: menuId,
                        user_id: userId
                    })
                })
                .then(response => response.json())
                .then(data => {
                    if (data.error) {
                        showMessage('error', data.error);
                    } else {
                        showMessage('success', '✅ Запись на выдачу создана! Теперь нажмите "Выдать" в списке ожидающих.');
                        setTimeout(() => location.reload(), 1500);
                    }
                })
                .catch(error => {
                    showMessage('error', '❌ Ошибка: ' + error);
                });
            }
            
            function showMessage(type, content) {
                const messageDiv = document.getElementById('message');
                if (type === 'error') {
                    messageDiv.innerHTML = '<div style="background: #f8d7da; color: #721c24; padding: 20px; border-radius: 10px; border-left: 5px solid #e74c3c;">' + content + '</div>';
                } else {
                    messageDiv.innerHTML = '<div style="background: #d4edda; color: #155724; padding: 20px; border-radius: 10px; border-left: 5px solid #27ae60;">' + content + '</div>';
                }
                messageDiv.scrollIntoView({behavior: 'smooth'});
            }
        </script>
    </body>
    </html>
    '''
    
    return html

def admin_dashboard():
    """Панель администратора"""
    db = get_db()
    today = date.today().strftime('%Y-%m-%d')
    
    # Получаем информацию о пользователе
    user = db.execute('SELECT * FROM users WHERE id = ?', (session.get('user_id'),)).fetchone()
    if not user:
        session.clear()
        return redirect('/')
    
    # Форматируем телефон для отображения
    phone_display = user['phone'] if user['phone'] else 'Не указан'
    
    # Статистика
    total_users = db.execute('SELECT COUNT(*) as count FROM users').fetchone()['count']
    total_students = db.execute('SELECT COUNT(*) as count FROM users WHERE role = ?', ('student',)).fetchone()['count']
    total_chefs = db.execute('SELECT COUNT(*) as count FROM users WHERE role = ?', ('chef',)).fetchone()['count']
    total_admins = db.execute('SELECT COUNT(*) as count FROM users WHERE role = ?', ('admin',)).fetchone()['count']
    total_orders = db.execute('SELECT COUNT(*) as count FROM orders').fetchone()['count']
    total_subscriptions = db.execute('SELECT COUNT(*) as count FROM subscriptions').fetchone()['count']
    total_reviews = db.execute('SELECT COUNT(*) as count FROM reviews').fetchone()['count']
    total_issued = db.execute('SELECT COUNT(*) as count FROM issued_meals WHERE issue_date = ?', (today,)).fetchone()['count']
    
    # Финансовая статистика
    total_orders_revenue = db.execute('SELECT SUM(m.price) as total FROM orders o JOIN menu m ON o.menu_id = m.id').fetchone()['total'] or 0
    total_subscriptions_revenue = db.execute('SELECT SUM(total_price) as total FROM subscriptions').fetchone()['total'] or 0
    total_revenue = total_orders_revenue + total_subscriptions_revenue
    
    # Сегодняшняя выручка
    today_orders_revenue = db.execute('''
        SELECT SUM(m.price) as total FROM orders o 
        JOIN menu m ON o.menu_id = m.id 
        WHERE DATE(o.created_at) = ?
    ''', (today,)).fetchone()['total'] or 0
    
    today_subscriptions_revenue = db.execute('''
        SELECT SUM(total_price) as total FROM subscriptions 
        WHERE DATE(created_at) = ?
    ''', (today,)).fetchone()['total'] or 0
    
    # Последние пользователи
    recent_users = db.execute('SELECT * FROM users ORDER BY created_at DESC LIMIT 5').fetchall()
    
    # Последние абонементы
    recent_subscriptions = db.execute('''
        SELECT s.*, u.first_name, u.last_name, m.name as menu_name
        FROM subscriptions s
        JOIN users u ON s.user_id = u.id
        JOIN menu m ON s.menu_id = m.id
        ORDER BY s.created_at DESC
        LIMIT 5
    ''').fetchall()
    
    # Статистика по выдаче за сегодня
    issued_stats = db.execute('''
        SELECT m.meal_type, COUNT(*) as count
        FROM issued_meals im
        JOIN menu m ON im.menu_id = m.id
        WHERE im.issue_date = ? AND im.status = 'выдан'
        GROUP BY m.meal_type
    ''', (today,)).fetchall()
    
    html = f'''
    <!DOCTYPE html>
    <html>
    <head>
        <title>Панель администратора</title>
        <meta charset="utf-8">
        <style>
            body {{
                font-family: 'Arial', sans-serif;
                background: #f5f5f5;
                margin: 0;
                padding: 20px;
            }}
            .header {{
                background: linear-gradient(135deg, #2c3e50 0%, #34495e 100%);
                color: white;
                padding: 30px;
                border-radius: 15px;
                margin-bottom: 30px;
                box-shadow: 0 10px 30px rgba(0,0,0,0.2);
            }}
            .stats-grid {{
                display: grid;
                grid-template-columns: repeat(auto-fit, minmax(200px, 1fr));
                gap: 20px;
                margin-bottom: 30px;
            }}
            .stat-card {{
                background: white;
                padding: 20px;
                border-radius: 10px;
                text-align: center;
                box-shadow: 0 3px 10px rgba(0,0,0,0.1);
            }}
            .stat-value {{
                font-size: 2em;
                font-weight: bold;
                margin: 10px 0;
            }}
            .stat-orders {{ color: #3498db; }}
            .stat-subscriptions {{ color: #9b59b6; }}
            .stat-revenue {{ color: #27ae60; }}
            .stat-users {{ color: #e67e22; }}
            .dashboard-grid {{
                display: grid;
                grid-template-columns: 1fr 1fr;
                gap: 30px;
            }}
            .section {{
                background: white;
                padding: 25px;
                border-radius: 15px;
                box-shadow: 0 5px 15px rgba(0,0,0,0.1);
            }}
            table {{
                width: 100%;
                border-collapse: collapse;
                margin-top: 15px;
            }}
            th, td {{
                padding: 12px;
                text-align: left;
                border-bottom: 1px solid #ddd;
            }}
            th {{
                background: #f8f9fa;
            }}
            .role-badge {{
                padding: 5px 10px;
                border-radius: 15px;
                font-size: 0.8em;
            }}
            .role-student {{ background: #3498db; color: white; }}
            .role-chef {{ background: #e67e22; color: white; }}
            .role-admin {{ background: #2c3e50; color: white; }}
            .subscription-card {{
                background: #f8f9fa;
                padding: 15px;
                border-radius: 10px;
                margin: 10px 0;
                border-left: 4px solid #9b59b6;
            }}
            .logout-btn {{
                background: #e74c3c;
                color: white;
                padding: 10px 20px;
                border: none;
                border-radius: 8px;
                cursor: pointer;
                text-decoration: none;
                display: inline-block;
                margin-left: 20px;
                transition: background 0.3s;
            }}
            .logout-btn:hover {{
                background: #c0392b;
            }}
            .progress-bar {{
                height: 20px;
                background: #ecf0f1;
                border-radius: 10px;
                margin: 10px 0;
                overflow: hidden;
            }}
            .progress-fill {{
                height: 100%;
                border-radius: 10px;
            }}
            .progress-breakfast {{ background: #3498db; }}
            .progress-lunch {{ background: #e67e22; }}
            .progress-snack {{ background: #9b59b6; }}
            .phone-info {{
                background: rgba(255, 255, 255, 0.2);
                padding: 8px 15px;
                border-radius: 8px;
                display: inline-block;
                margin: 10px 0;
            }}
        </style>
    </head>
    <body>
        <div class="header">
            <h1>👨‍💼 Панель администратора</h1>
            <p>Добро пожаловать, {user['first_name']} {user['last_name']}!</p>
            <div class="phone-info">
                📱 <strong>Телефон:</strong> {phone_display}
            </div>
            <p><strong>Должность:</strong> {user['position']} | <strong>Дата:</strong> {today}</p>
            <a href="/logout" class="logout-btn">Выйти</a>
        </div>
        
        <h2>📊 Статистика системы:</h2>
        
        <div class="stats-grid">
            <div class="stat-card">
                <div class="stat-value stat-users">{total_users}</div>
                <div>Всего пользователей</div>
            </div>
            <div class="stat-card">
                <div class="stat-value stat-orders">{total_orders}</div>
                <div>Разовых заказов</div>
            </div>
            <div class="stat-card">
                <div class="stat-value stat-subscriptions">{total_subscriptions}</div>
                <div>Абонементов</div>
            </div>
            <div class="stat-card">
                <div class="stat-value stat-revenue">{total_revenue} руб.</div>
                <div>Общая выручка</div>
            </div>
            <div class="stat-card">
                <div class="stat-value">{total_issued}</div>
                <div>Выдано сегодня</div>
            </div>
            <div class="stat-card">
                <div class="stat-value stat-revenue">{today_orders_revenue + today_subscriptions_revenue} руб.</div>
                <div>Выручка сегодня</div>
            </div>
        </div>
        
        <div class="dashboard-grid">
            <div class="section">
                <h2>👥 Последние пользователи:</h2>
                <table>
                    <thead>
                        <tr>
                            <th>Имя</th>
                            <th>Email</th>
                            <th>Роль</th>
                            <th>Дата регистрации</th>
                        </tr>
                    </thead>
                    <tbody>
    '''
    
    for user in recent_users:
        role_class = f"role-{user['role']}"
        html += f'''
                        <tr>
                            <td>{user['first_name']} {user['last_name']}</td>
                            <td>{user['email']}</td>
                            <td><span class="role-badge {role_class}">{user['role']}</span></td>
                            <td>{user['created_at'][:10] if user['created_at'] else 'не указано'}</td>
                        </tr>
        '''
    
    html += '''
                    </tbody>
                </table>
            </div>
            
            <div class="section">
                <h2>📋 Последние абонементы:</h2>
    '''
    
    if recent_subscriptions:
        for sub in recent_subscriptions:
            selected_dates = json.loads(sub['selected_dates'])
            html += f'''
                <div class="subscription-card">
                    <h4>{sub['menu_name']}</h4>
                    <p><strong>Ученик:</strong> {sub['first_name']} {sub['last_name']}</p>
                    <p><strong>Период:</strong> {sub['start_date']} - {sub['end_date']}</p>
                    <p><strong>Количество дней:</strong> {len(selected_dates)}</p>
                    <p><strong>Стоимость:</strong> {sub['total_price']} руб.</p>
                    <p><strong>Статус:</strong> {sub['status']}</p>
                </div>
            '''
    else:
        html += '<p>Абонементов пока нет.</p>'
    
    html += '''
            </div>
        </div>
        
        <div class="section" style="margin-top: 30px;">
            <h2>📈 Статистика выдачи на сегодня:</h2>
    '''
    
    if issued_stats:
        for stat in issued_stats:
            meal_type_ru = {
                'завтрак': 'Завтраки',
                'обед': 'Обеды',
                'полдник': 'Полдники'
            }.get(stat['meal_type'], stat['meal_type'])
            
            progress_class = {
                'завтрак': 'progress-breakfast',
                'обед': 'progress-lunch',
                'полдник': 'progress-snack'
            }.get(stat['meal_type'], 'progress-breakfast')
            
            html += f'''
                <div style="margin: 15px 0;">
                    <p><strong>{meal_type_ru}:</strong> {stat['count']} порций</p>
                    <div class="progress-bar">
                        <div class="progress-fill {progress_class}" style="width: {min(stat['count'] * 10, 100)}%;"></div>
                    </div>
                </div>
            '''
    else:
        html += '<p>На сегодня статистики выдачи нет.</p>'
    
    html += '''
        </div>
    </body>
    </html>
    '''
    
    return html

# ==================== API ====================

@app.route('/api/order', methods=['POST'])
def create_order():
    """Создание заказа через API"""
    if 'user_id' not in session or session.get('user_role') != 'student':
        return jsonify({'error': 'Не авторизован как ученик'}), 401
    
    data = request.json
    if not data or 'menu_id' not in data:
        return jsonify({'error': 'Не указан menu_id'}), 400
    
    db = get_db()
    
    # Получаем пользователя
    user = db.execute('SELECT * FROM users WHERE id = ?', (session.get('user_id'),)).fetchone()
    if not user:
        return jsonify({'error': 'Пользователь не найден'}), 404
    
    # Получаем блюдо
    menu_item = db.execute('SELECT * FROM menu WHERE id = ?', (data['menu_id'],)).fetchone()
    if not menu_item:
        return jsonify({'error': 'Блюдо не найдено'}), 404
    
    # Проверяем баланс
    if user['balance'] < menu_item['price']:
        return jsonify({'error': 'Недостаточно средств на балансе'}), 400
    
    # Создаем заказ
    order_id = str(uuid.uuid4())
    db.execute(
        'INSERT INTO orders (id, user_id, menu_id, status) VALUES (?, ?, ?, ?)',
        (order_id, user['id'], data['menu_id'], 'ожидает оплаты')
    )
    
    # Создаем запись о выдаче
    issue_id = str(uuid.uuid4())
    today = date.today().strftime('%Y-%m-%d')
    db.execute(
        'INSERT INTO issued_meals (id, user_id, menu_id, issue_date, meal_type, status) VALUES (?, ?, ?, ?, ?, ?)',
        (issue_id, user['id'], data['menu_id'], today, menu_item['meal_type'], 'ожидает выдачи')
    )
    
    # Списываем средства
    new_balance = user['balance'] - menu_item['price']
    db.execute('UPDATE users SET balance = ? WHERE id = ?', (new_balance, user['id']))
    
    db.commit()
    
    return jsonify({
        'success': True,
        'order_id': order_id,
        'new_balance': new_balance
    })

@app.route('/api/subscription', methods=['POST'])
def create_subscription():
    """Создание абонемента"""
    if 'user_id' not in session or session.get('user_role') != 'student':
        return jsonify({'error': 'Не авторизован как ученик'}), 401
    
    data = request.json
    if not data or 'menu_id' not in data or 'selected_dates' not in data:
        return jsonify({'error': 'Не указаны обязательные данные'}), 400
    
    selected_dates = data['selected_dates']
    if not isinstance(selected_dates, list) or len(selected_dates) == 0:
        return jsonify({'error': 'Выберите хотя бы одну дату'}), 400
    
    db = get_db()
    
    # Получаем пользователя
    user = db.execute('SELECT * FROM users WHERE id = ?', (session.get('user_id'),)).fetchone()
    if not user:
        return jsonify({'error': 'Пользователь не найден'}), 404
    
    # Получаем блюдо
    menu_item = db.execute('SELECT * FROM menu WHERE id = ?', (data['menu_id'],)).fetchone()
    if not menu_item:
        return jsonify({'error': 'Блюдо не найдено'}), 404
    
    # Проверяем, не оформлен ли уже абонемент на эти даты
    existing_subs = db.execute('SELECT selected_dates FROM subscriptions WHERE user_id = ? AND menu_id = ? AND status = "активен"', 
                              (session.get('user_id'), data['menu_id'])).fetchall()
    
    for sub in existing_subs:
        existing_dates = json.loads(sub['selected_dates'])
        for date_str in selected_dates:
            if date_str in existing_dates:
                return jsonify({'error': f'У вас уже есть абонемент на {date_str}'}), 400
    
    # Проверяем баланс
    total_price = data.get('total_price', menu_item['price'] * len(selected_dates))
    if user['balance'] < total_price:
        return jsonify({'error': f'Недостаточно средств на балансе. Нужно: {total_price} руб., есть: {user["balance"]} руб.'}), 400
    
    # Создаем абонемент
    subscription_id = str(uuid.uuid4())
    db.execute(
        '''INSERT INTO subscriptions (id, user_id, menu_id, selected_dates, start_date, end_date, total_price, status) 
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
        (subscription_id, user['id'], data['menu_id'], json.dumps(selected_dates, ensure_ascii=False), 
         data['start_date'], data['end_date'], total_price, 'активен')
    )
    
    # Создаем записи о выдаче для каждой даты
    for date_str in selected_dates:
        issue_id = str(uuid.uuid4())
        db.execute(
            '''INSERT INTO issued_meals (id, user_id, menu_id, subscription_id, issue_date, meal_type, status) 
               VALUES (?, ?, ?, ?, ?, ?, ?)''',
            (issue_id, user['id'], data['menu_id'], subscription_id, date_str, menu_item['meal_type'], 'ожидает выдачи')
        )
    
    # Списываем средства
    new_balance = user['balance'] - total_price
    db.execute('UPDATE users SET balance = ? WHERE id = ?', (new_balance, user['id']))
    
    db.commit()
    
    return jsonify({
        'success': True,
        'subscription_id': subscription_id,
        'days_count': len(selected_dates),
        'total_price': total_price,
        'new_balance': new_balance
    })

@app.route('/api/issue-meal', methods=['POST'])
def issue_meal():
    """Выдача блюда поваром"""
    if 'user_id' not in session or session.get('user_role') != 'chef':
        return jsonify({'error': 'Не авторизован как повар'}), 401
    
    data = request.json
    if not data or 'meal_id' not in data:
        return jsonify({'error': 'Не указан meal_id'}), 400
    
    db = get_db()
    
    # Проверяем существование записи
    meal = db.execute('SELECT * FROM issued_meals WHERE id = ?', (data['meal_id'],)).fetchone()
    if not meal:
        return jsonify({'error': 'Запись о выдаче не найдена'}), 404
    
    if meal['status'] == 'выдан':
        return jsonify({'error': 'Блюдо уже выдано'}), 400
    
    # Обновляем статус и указываем, кто выдал
    db.execute(
        'UPDATE issued_meals SET status = ?, issued_by = ? WHERE id = ?',
        ('выдан', session.get('user_id'), data['meal_id'])
    )
    
    db.commit()
    
    return jsonify({
        'success': True,
        'message': 'Блюдо успешно выдано'
    })

@app.route('/api/issue-from-subscription', methods=['POST'])
def issue_from_subscription():
    """Создание записи о выдаче по абонементу"""
    if 'user_id' not in session or session.get('user_role') != 'chef':
        return jsonify({'error': 'Не авторизован как повар'}), 401
    
    data = request.json
    if not data or 'subscription_id' not in data or 'menu_id' not in data or 'user_id' not in data:
        return jsonify({'error': 'Не указаны обязательные данные'}), 400
    
    db = get_db()
    today = date.today().strftime('%Y-%m-%d')
    
    # Проверяем, есть ли уже запись на сегодня
    existing_issue = db.execute('''
        SELECT * FROM issued_meals 
        WHERE user_id = ? AND menu_id = ? AND issue_date = ? AND subscription_id = ?
    ''', (data['user_id'], data['menu_id'], today, data['subscription_id'])).fetchone()
    
    if existing_issue:
        return jsonify({'error': 'Запись на выдачу уже существует'}), 400
    
    # Получаем информацию о блюде
    menu_item = db.execute('SELECT * FROM menu WHERE id = ?', (data['menu_id'],)).fetchone()
    if not menu_item:
        return jsonify({'error': 'Блюдо не найдено'}), 404
    
    # Создаем запись о выдаче
    issue_id = str(uuid.uuid4())
    db.execute(
        '''INSERT INTO issued_meals (id, user_id, menu_id, subscription_id, issue_date, meal_type, status) 
           VALUES (?, ?, ?, ?, ?, ?, ?)''',
        (issue_id, data['user_id'], data['menu_id'], data['subscription_id'], today, menu_item['meal_type'], 'ожидает выдачи')
    )
    
    db.commit()
    
    return jsonify({
        'success': True,
        'message': 'Запись на выдачу создана'
    })

@app.route('/api/review', methods=['POST'])
def create_review():
    """Создание отзыва"""
    if 'user_id' not in session:
        return jsonify({'error': 'Не авторизован'}), 401
    
    data = request.json
    if not data or 'menu_id' not in data or 'rating' not in data:
        return jsonify({'error': 'Не указаны menu_id или rating'}), 400
    
    rating = int(data['rating'])
    if rating < 1 or rating > 5:
        return jsonify({'error': 'Рейтинг должен быть от 1 до 5'}), 400
    
    db = get_db()
    
    # Проверяем, существует ли блюдо
    menu_item = db.execute('SELECT * FROM menu WHERE id = ?', (data['menu_id'],)).fetchone()
    if not menu_item:
        return jsonify({'error': 'Блюдо не найдено'}), 404
    
    # Проверяем, не оставлял ли пользователь уже отзыв на это блюдо
    existing_review = db.execute('SELECT * FROM reviews WHERE user_id = ? AND menu_id = ?', 
                                (session.get('user_id'), data['menu_id'])).fetchone()
    
    if existing_review:
        return jsonify({'error': 'Вы уже оставляли отзыв на это блюдо'}), 400
    
    # Создаем отзыв
    review_id = str(uuid.uuid4())
    db.execute(
        'INSERT INTO reviews (id, user_id, menu_id, rating, comment) VALUES (?, ?, ?, ?, ?)',
        (review_id, session.get('user_id'), data['menu_id'], rating, data.get('comment', ''))
    )
    
    db.commit()
    
    return jsonify({
        'success': True,
        'review_id': review_id,
        'message': 'Отзыв успешно добавлен'
    })

@app.route('/api/review/<review_id>', methods=['GET', 'PUT', 'DELETE'])
def manage_review(review_id):
    """Получение, обновление или удаление отзыва"""
    if 'user_id' not in session:
        return jsonify({'error': 'Не авторизован'}), 401
    
    db = get_db()
    
    if request.method == 'GET':
        # Получение отзыва
        review = db.execute('SELECT * FROM reviews WHERE id = ?', (review_id,)).fetchone()
        if not review:
            return jsonify({'error': 'Отзыв не найден'}), 404
        
        if review['user_id'] != session.get('user_id') and session.get('user_role') not in ['admin', 'chef']:
            return jsonify({'error': 'Нет прав на просмотр этого отзыва'}), 403
        
        return jsonify({
            'id': review['id'],
            'menu_id': review['menu_id'],
            'rating': review['rating'],
            'comment': review['comment'],
            'created_at': review['created_at']
        })
    
    elif request.method == 'PUT':
        # Обновление отзыва
        data = request.json
        if not data:
            return jsonify({'error': 'Нет данных для обновления'}), 400
        
        review = db.execute('SELECT * FROM reviews WHERE id = ?', (review_id,)).fetchone()
        if not review:
            return jsonify({'error': 'Отзыв не найден'}), 404
        
        if review['user_id'] != session.get('user_id'):
            return jsonify({'error': 'Нет прав на редактирование этого отзыва'}), 403
        
        rating = data.get('rating', review['rating'])
        comment = data.get('comment', review['comment'])
        
        if rating < 1 or rating > 5:
            return jsonify({'error': 'Рейтинг должен быть от 1 до 5'}), 400
        
        db.execute(
            'UPDATE reviews SET rating = ?, comment = ? WHERE id = ?',
            (rating, comment, review_id)
        )
        
        db.commit()
        
        return jsonify({
            'success': True,
            'message': 'Отзыв успешно обновлен'
        })
    
    elif request.method == 'DELETE':
        # Удаление отзыва
        review = db.execute('SELECT * FROM reviews WHERE id = ?', (review_id,)).fetchone()
        if not review:
            return jsonify({'error': 'Отзыв не найден'}), 404
        
        if review['user_id'] != session.get('user_id') and session.get('user_role') != 'admin':
            return jsonify({'error': 'Нет прав на удаление этого отзыва'}), 403
        
        db.execute('DELETE FROM reviews WHERE id = ?', (review_id,))
        db.commit()
        
        return jsonify({
            'success': True,
            'message': 'Отзыв успешно удален'
        })

# ==================== ВЫХОД ====================

@app.route('/logout')
def logout():
    """Выход из системы"""
    session.clear()
    return '''
    <!DOCTYPE html>
    <html>
    <head>
        <title>Выход</title>
        <style>
            body { 
                font-family: Arial; 
                text-align: center; 
                padding: 100px; 
                background: #f0f8ff;
            }
            .message {
                background: white;
                padding: 40px;
                border-radius: 15px;
                box-shadow: 0 10px 30px rgba(0,0,0,0.1);
                display: inline-block;
            }
            .btn {
                background: #3498db;
                color: white;
                padding: 12px 25px;
                border-radius: 8px;
                text-decoration: none;
                display: inline-block;
                margin-top: 20px;
            }
        </style>
    </head>
    <body>
        <div class="message">
            <h2>👋 Вы вышли из системы</h2>
            <p>Спасибо за использование системы!</p>
            <a href="/" class="btn">На главную</a>
        </div>
    </body>
    </html>
    '''

# ==================== ЗАПУСК ====================
@app.before_request
def before_request():
    """Подключение к БД перед каждым запросом"""
    g.db = get_db()

@app.teardown_appcontext
def close_connection(exception):
    """Закрытие соединения с БД после запроса"""
    db = g.pop('db', None)
    if db is not None:
        db.close()

if __name__ == '__main__':
    # Инициализация БД
    with app.app_context():
        init_db()
    
    print("=" * 70)
    print("🚀 ШКОЛЬНАЯ СТОЛОВАЯ ЗАПУЩЕНА!")
    print("=" * 70)
    print("✨ ОБНОВЛЕНИЯ:")
    print("✅ УДАЛЕНЫ все тестовые аккаунты")
    print("✅ ДОБАВЛЕН номер телефона при регистрации")
    print("✅ УЛУЧШЕНА валидация данных")
    print("=" * 70)
    
    print("\n📡 Доступ к сайту:")
    print("1. Этот компьютер: http://localhost:5000")
    print("2. Другие в вашей Wi-Fi сети: http://ВАШ_IP:5000")
    
    # Показываем IP адрес
    import socket
    try:
        s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        s.connect(("8.8.8.8", 80))
        ip = s.getsockname()[0]
        s.close()
        print(f"3. Ваш IP адрес: {ip}")
        print(f"   Для друзей в вашей сети: http://{ip}:5000")
    except:
        print("3. Чтобы узнать IP: откройте cmd и введите 'ipconfig'")
    
    print("=" * 70)
    print("👨‍🎓 УЧЕНИК: регистрация через сайт (требуется телефон)")
    print("👨‍🍳 ПОВАР: регистрация через сайт (требуется телефон)")
    print("👨‍💼 АДМИН: регистрация через сайт (требуется телефон)")
    print("=" * 70)
    
    # Запуск Flask
    app.run(host='0.0.0.0', port=5000, debug=True)